...
impl ModConstants {
    ...
    pub fn merge(&mut self, other: &ModConstants) {
        ...
        if other.max_xp_from_barbarians != defaults.max_xp_from_barbarians {
            this.max_xp_from_barbarians = other.max_xp_from_barbarians;
        }

        if other.city_strength_base != defaults.city_strength_base {
            this.city_strength_base = other.city_strength_base;
        }

        if other.city_strength_per_pop != defaults.city_strength_per_pop {
            this.city_strength_per_pop = other.city_strength_per_pop;
        }

        if other.city_strength_from_techs_multiplier != defaults.city_strength_from_techs_multiplier {
            this.city_strength_from_techs_multiplier = other.city_strength_from_techs_multiplier;
        }

        if other.city_strength_from_techs_exponent != defaults.city_strength_from_techs_exponent {
            this.city_strength_from_techs_exponent = other.city_strength_from_techs_exponent;
        }

        if other.city_strength_from_techs_full_multiplier != defaults.city_strength_from_techs_full_multiplier {
            this.city_strength_from_techs_full_multiplier = other.city_strength_from_techs_full_multiplier;
        }

        if other.city_strength_from_garrison != defaults.city_strength_from_garrison {
            this.city_strength_from_garrison = other.city_strength_from_garrison;
        }

        if other.unit_supply_per_population != defaults.unit_supply_per_population {
            this.unit_supply_per_population = other.unit_supply_per_population;
        }

        if other.minimal_city_distance != defaults.minimal_city_distance {
            this.minimal_city_distance = other.minimal_city_distance;
        }

        if other.minimal_city_distance_on_different_continents != defaults.minimal_city_distance_on_different_continents {
            this.minimal_city_distance_on_different_continents = other.minimal_city_distance_on_different_continents;
        }

        if other.base_city_bombard_range != defaults.base_city_bombard_range {
            this.base_city_bombard_range = other.base_city_bombard_range;
        }

        if other.city_work_range != defaults.city_work_range {
            this.city_work_range = other.city_work_range;
        }

        if other.city_expand_range != defaults.city_expand_range {
            this.city_expand_range = other.city_expand_range;
        }

        if other.gold_gift_multiplier != defaults.gold_gift_multiplier {
            this.gold_gift_multiplier = other.gold_gift_multiplier;
        }

        if other.gold_gift_trade_multiplier != defaults.gold_gift_trade_multiplier {
            this.gold_gift_trade_multiplier = other.gold_gift_trade_multiplier;
        }

        if other.gold_gift_degradation_multiplier != defaults.gold_gift_degradation_multiplier {
            this.gold_gift_degradation_multiplier = other.gold_gift_degradation_multiplier;
        }

        if other.unit_upgrade_cost != defaults.unit_upgrade_cost {
            this.unit_upgrade_cost = other.unit_upgrade_cost.clone();
        }

        if other.natural_wonder_count_multiplier != defaults.natural_wonder_count_multiplier {
            this.natural_wonder_count_multiplier = other.natural_wonder_count_multiplier;
        }

        if other.natural_wonder_count_added_constant != defaults.natural_wonder_count_added_constant {
            this.natural_wonder_count_added_constant = other.natural_wonder_count_added_constant;
        }

        if other.ancient_ruin_count_multiplier != defaults.ancient_ruin_count_multiplier {
            this.ancient_ruin_count_multiplier = other.ancient_ruin_count_multiplier;
        }

        if other.spawn_ice_below_temperature != defaults.spawn_ice_below_temperature {
            this.spawn_ice_below_temperature = other.spawn_ice_below_temperature;
        }

        if other.max_lake_size != defaults.max_lake_size {
            this.max_lake_size = other.max_lake_size;
        }

        if other.river_count_multiplier != defaults.river_count_multiplier {
            this.river_count_multiplier = other.river_count_multiplier;
        }

        if other.min_river_length != defaults.min_river_length {
            this.min_river_length = other.min_river_length;
        }

        if other.max_river_length != defaults.max_river_length {
            this.max_river_length = other.max_river_length;
        }

        if other.religion_limit_base != defaults.religion_limit_base {
            this.religion_limit_base = other.religion_limit_base;
        }

        if other.religion_limit_multiplier != defaults.religion_limit_multiplier {
            this.religion_limit_multiplier = other.religion_limit_multiplier;
        }

        if other.pantheon_base != defaults.pantheon_base {
            this.pantheon_base = other.pantheon_base;
        }

        if other.pantheon_growth != defaults.pantheon_growth {
            this.pantheon_growth = other.pantheon_growth;
        }

        if other.workboat_automation_search_max_tiles != defaults.workboat_automation_search_max_tiles {
            this.workboat_automation_search_max_tiles = other.workboat_automation_search_max_tiles;
        }

        if other.minimum_war_duration != defaults.minimum_war_duration {
            this.minimum_war_duration = other.minimum_war_duration;
        }

        if other.base_turns_until_revolt != defaults.base_turns_until_revolt {
            this.base_turns_until_revolt = other.base_turns_until_revolt;
        }

        if other.city_state_election_turns != defaults.city_state_election_turns {
            this.city_state_election_turns = other.city_state_election_turns;
        }

        if other.tribute_global_modifier != defaults.tribute_global_modifier {
            this.tribute_global_modifier = other.tribute_global_modifier;
        }

        if other.tribute_local_modifier != defaults.tribute_local_modifier {
            this.tribute_local_modifier = other.tribute_local_modifier;
        }

        if other.max_spy_rank != defaults.max_spy_rank {
            this.max_spy_rank = other.max_spy_rank;
        }

        if other.spy_rank_skill_percent_bonus != defaults.spy_rank_skill_percent_bonus {
            this.spy_rank_skill_percent_bonus = other.spy_rank_skill_percent_bonus;
        }

        if other.spy_rank_steal_percent_bonus != defaults.spy_rank_steal_percent_bonus {
            this.spy_rank_steal_percent_bonus = other.spy_rank_steal_percent_bonus;
        }

        if other.spy_tech_steal_cost_modifier != defaults.spy_tech_steal_cost_modifier {
            this.spy_tech_steal_cost_modifier = other.spy_tech_steal_cost_modifier;
        }

        if other.score_from_population != defaults.score_from_population {
            this.score_from_population = other.score_from_population;
        }

        if other.score_from_wonders != defaults.score_from_wonders {
            this.score_from_wonders = other.score_from_wonders;
        }

        if other.max_improvement_tech_eras_forward != defaults.max_improvement_tech_eras_forward {
            this.max_improvement_tech_eras_forward = other.max_improvement_tech_eras_forward;
        }
    }
}
...
impl PartialEq for ModConstants {
    fn eq(&self, other: &Self) -> bool {
        ...
        self.city_strength_from_techs_full_multiplier == other.city_strength_from_techs_full_multiplier &&
        ...
        self.minimal_city_distance_on_different_continents == other.minimal_city_distance_on_different_continents &&
...
impl Hash for ModConstants {
    fn hash<H: Hasher>(&self, state: &mut H) {
        ...
        self.city_strength_from_techs_multiplier.to_bits().hash(state);
        self.city_strength_from_techs_exponent.to_bits().hash(state);
        self.city_strength_from_techs_full_multiplier.to_bits().hash(state);
        ...
        self.minimal_city_distance_on_different_continents.hash(state);
        ...
        self.natural_wonder_count_added_constant.to_bits().hash(state);
