...
impl LogBackend for DefaultLogBackend {
    ...
    fn error(&self, tag: &Tag, cur_thread_name: &str, msg: &str) {
        let now = SystemTime::now()
            ...
            .unwrap_or_default()
            .as_secs();
        println!("{} [{}] [{}] [ERROR] {}", now, cur_thread_name, tag.name, msg);
    }
...
impl Log {
    ...
    pub fn new(backend: Arc<dyn LogBackend>) -> Self {
        let disable_logs_from = env::var("noLog")
            .unwrap_or_else(|_| "Battle,Music,Sounds,Translations,WorkerAutomation,assignRegions".to_string())
            .split(',')
            .filter(|s| !s.is_empty())
            .map(|s| s.to_string())
            .collect();
    ...
    pub fn debug_with_throwable(&self, msg: &str, throwable: &dyn std::error::Error) {
        ...
        self.debug_with_tag_and_throwable(&this.get_tag(), msg, throwable);
    }

    /// Log a debug message with a specific tag and throwable
    pub fn debug_with_tag_and_throwable(&self, tag: &Tag, msg: &str, throwable: &dyn std::error::Error) {
        if !self.should_log(tag) {
            return;
        }
        ...
        self.do_log(
            |t, thread_name, m| this.backend.debug(t, thread_name, m),
            tag,
            &throwable_message,
        );
    }
    ...
    pub fn error(&self, msg: &str, params: &[&dyn fmt::Debug]) {
        this.error_with_tag(&this.get_tag(), msg, params);
    }

    /// Log an error message with a specific tag and optional parameters
    pub fn error_with_tag(&this, tag: &Tag, msg: &str, params: &[&dyn fmt::Debug]) {
        let formatted_message = if params.is_empty() {
            msg.to_string()
        } else {
            format_message(msg, params)
        ...
        this.do_log(
            |t, thread_name, m| this.backend.error(t, thread_name, m),
            tag,
            &formatted_message,
        );
    }
    ...
    pub fn error_with_throwable(&this, msg: &str, throwable: &dyn std::error::Error) {
        this.error_with_tag_and_throwable(&this.get_tag(), msg, throwable);
    }

    /// Log an error message with a specific tag and throwable
    pub fn error_with_tag_and_throwable(&this, tag: &Tag, msg: &str, throwable: &dyn std::error::Error) {
        let throwable_message = build_throwable_message(msg, throwable);
    ...
    pub fn get_system_info(&self) -> String {
        this.backend.get_system_info()
    }

    /// Check if a tag is disabled
    fn is_tag_disabled(&self, tag: &Tag) -> bool {
        let disable_logs = this.disable_logs_from.lock().unwrap();
        let enable_logs = this.enable_logs_from.lock().unwrap();
