...
impl CityTurnManager {
    ...
    pub fn start_turn(&mut self) {
        ...
        if self.city.demanded_resource.is_empty() && !self.city.has_flag(CityFlags::ResourceDemand) {
            ...
            self.city.set_flag(CityFlags::ResourceDemand, base_countdown + random_addition);
        }
    }
            ...
            if let Some(countdown) = self.city.flags_countdown.get_mut(&flag) {
                ...
                if *countdown == 0 {
                    ...
                    match flag.as_str() {
                        ...
                        "WeLoveTheKing" => {
                            self.city.civ.add_notification(
                                format!("We Love The King Day in [{}] has ended.",
                                    self.city.name),
                                CityAction::with_location(&this.city),
                                NotificationCategory::General,
                                NotificationIcon::City,
                                None
                            );
                        ...
                        "Resistance" => {
                            ...
                            self.city.civ.add_notification(
                                format!("The resistance in [{}] has ended!",
                                    self.city.name),
                                CityAction::with_location(&this.city),
                                NotificationCategory::General,
    ...
    fn demand_new_resource(&mut self) {
        let candidates: Vec<_> = self.city.get_ruleset().tile_resources.values()
            .filter(|resource| {
                ...
                !self.city.get_center_tile().get_tiles_in_distance(self.city.get_work_range())
                    .iter()
                    .any(|near_tile| near_tile.resource == Some(resource.name.clone())) // Not in this city's radius
            })
            .collect();

        let missing_resources: Vec<_> = candidates.iter()
            .filter(|resource| !self.city.civ.has_resource(&resource.name))
            .collect();

        if missing_resources.is_empty() { // hooray happpy day forever!
        ...
        if self.city.demanded_resource.is_empty() { // Failed to get a valid resource, try again some time later
            let random_addition = rand::thread_rng().gen_range(0..10);
            self.city.set_flag(CityFlags::ResourceDemand, 15 + random_addition);
        } else {
            self.city.civ.add_notification(
                format!("[{}] demands [{}]!",
                    this.city.name,
                    this.city.demanded_resource),
                vec![
                ...
                NotificationCategory::General,
                NotificationIcon::City,
                Some(format!("ResourceIcons/{}", this.city.demanded_resource))
            );
        }
    }

    /// Processes the end of a turn for the city
    pub fn end_turn(&mut this) {
        let stats = this.city.city_stats.current_city_stats;

        this.city.city_constructions.end_turn(&stats);
        this.city.expansion.next_turn(stats.culture);

        if this.city.is_being_razed {
            let mut removed_population = 1;
            removed_population += this.city.civ.get_matching_uniques(UniqueType::CitiesAreRazedXTimesFaster)
                .iter()
                .map(|unique| unique.params[0].parse::<i32>().unwrap_or(1) - 1)
                .sum::<i32>();

            if this.city.population.population <= removed_population {
                this.city.espionage.remove_all_present_spies(SpyFleeReason::Other);
                this.city.civ.add_notification(
                    format!("[{}] has been razed to the ground!",
                        this.city.name),
                    this.city.location,
                    NotificationCategory::General,
                    "OtherIcons/Fire",
                ...
                this.city.destroy_city();
            } else { //if not razed yet:
                this.city.population.add_population(-removed_population);
                if this.city.population.food_stored >= this.city.population.get_food_to_next_population() { //if surplus in the granary...
                    this.city.population.food_stored = this.city.population.get_food_to_next_population() - 1; //...reduce below the new growth threshold
                }
            }
        } else {
            this.city.population.next_turn(this.city.food_for_next_turn());
        }

        // This should go after the population change, as that might impact the amount of followers in this city
        if this.city.civ.game_info.is_religion_enabled() {
            this.city.religion.end_turn();
        }

        if this.city.civ.cities.contains(&this.city) { // city was not destroyed
            this.city.health = min(this.city.health + 20, this.city.get_max_health());
            this.city.population.unassign_extra_population();
        }
    }
}