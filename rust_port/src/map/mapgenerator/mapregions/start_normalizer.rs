...
use rand::prelude::*;
use crate::constants::constants::terrain::water::FRESH_WATER;
...
impl StartNormalizer {
    ...
    pub fn normalize_start(
        ...
        for tile in start_tile.neighbors() {
            if let Some(last_terrain) = tile
                .terrain_feature_objects
                .iter()
        ...
        let food_bonuses_needed =
            Self::calculate_food_bonuses_needed(start_tile, is_minor_civ, ruleset, tile_map);
    ...
    fn normalize_production(
        ...
        let inner_production = start_tile
            .neighbors()
            .map(|t| Self::get_potential_yield(t, Stat::Production, false) as i32)
            .sum::<i32>();

        let outer_production = start_tile
            .get_tiles_at_distance(2)
            .map(|t| Self::get_potential_yield(t, Stat::Production, false) as i32)
            .sum::<i32>();

        // for very early production we ideally want tiles that also give food
        let early_production = start_tile
        ...
        if inner_production == 0
            || (inner_production < 2 && outer_production < 8)
            || (is_minor_civ && inner_production < 4)
        {
            let hill_spots: Vec<&Tile> = start_tile
                .neighbors()
                .filter(|t| {
                    t.is_land
                        && t.terrain_features.is_empty()
                        && !t.is_adjacent_to(&FRESH_WATER)
                        && !t.is_impassible()
                })
                .collect();

            if let Some(hill_spot) = hill_spots.choose(&mut rand::rng()) {
                let hill_equivalent = ruleset
                    .terrains
                    .values()
                    .find(|t| {
                        t.terrain_type == TerrainType::TerrainFeature
                            && t.production >= 2
        ...
        if !is_minor_civ && inner_production < 3 && early_production < 6 {
            ...
            let early_eras: Vec<_> = ruleset
                .eras
                .values()
                .filter(|e| e.era_number <= last_era_number / 3)
                .collect();

            let valid_resources: Vec<_> = ruleset
                .tile_resources
                .values()
                .filter(|r| {
                    r.resource_type == ResourceType::Strategic
                        && (r.revealed_by.is_none()
                            || early_eras.iter().any(|e| {
                                e.name
                                    == ruleset.technologies[r.revealed_by.as_ref().unwrap()].era()
                            }))
                })
                .collect();
    ...
    fn place_strategic_balance_resources(
        ...
        let mut candidate_tiles: Vec<&Tile> =
            start_tile.get_tiles_in_distance_range(1..=2).collect();
        candidate_tiles.extend(start_tile.get_tiles_at_distance(3));
        candidate_tiles.shuffle(&mut rand::thread_rng());

        for resource in ruleset
            .tile_resources
            .values()
    ...
    fn add_production_bonuses(start_tile: &Tile, ruleset: &Ruleset) {
        let mut grass_type_plots: Vec<&Tile> = start_tile
            .get_tiles_in_distance_range(1..=2)
            .filter(|t| {
                t.is_land &&
                      Self::get_potential_yield(t, Stat::Food, true) >= 2.0 && // Food neutral natively
                      Self::get_potential_yield(t, Stat::Production) == 0.0
            }) // Production can't even be improved
            .collect();

        let plains_type_plots: Vec<&Tile> = start_tile
            .get_tiles_in_distance_range(1..=2)
            .filter(|t| {
                t.is_land &&
                      Self::get_potential_yield(t, Stat::Food) >= 2.0 && // Something that can be improved to food neutral
                      Self::get_potential_yield(t, Stat::Production, true) >= 1.0
            }) // Some production natively
            .collect();
        ...
        let production_bonuses: Vec<_> = ruleset
            .tile_resources
            .values()
        ...
        if !production_bonuses.is_empty() {
            while production_bonuses_needed > 0 && !grass_type_plots.is_empty() {
                ...
                let valid_bonuses: Vec<_> = production_bonuses
                    .iter()
    ...
    fn calculate_food_bonuses_needed(
        ...
        let inner_native_two_food = start_tile
            .neighbors()
            .filter(|t| Self::get_potential_yield(t, Stat::Food, true) >= 2.0)
            .count() as i32;

        let outer_native_two_food = start_tile
            .get_tiles_at_distance(2)
            .filter(|t| Self::get_potential_yield(t, Stat::Food, true) >= 2.0)
            .count() as i32;
        ...
        let mut bonuses_needed = if minor_civ {
            ...
            match (
                total_food,
                inner_food,
                total_native_two_food,
                inner_native_two_food,
            ) {
                (0, t, _, _) if t < 4 => 5,
                (t, _, _, _) if t < 6 => 4,
        ...
        if !minor_civ && bonuses_needed < 3 && total_native_two_food == 0 {
            let two_food_terrain = ruleset
                .terrains
                .values()
                .find(|t| t.terrain_type == TerrainType::Land && t.food >= 2)
                .map(|t| t.name.clone());

            let candidate_inner_spots: Vec<&Tile> = start_tile
                .neighbors()
                .filter(|t| {
                    t.is_land
                        && !t.is_impassible()
            ...
            let candidate_outer_spots: Vec<&Tile> = start_tile
                .get_tiles_at_distance(2)
                .filter(|t| {
                    t.is_land
                        && !t.is_impassible()
            ...
            let spot = candidate_inner_spots
    ...
    fn place_food_bonuses(
        ...
        let oasis_equivalent = ruleset
            ...
            .find(|t| {
                t.terrain_type == TerrainType::TerrainFeature
                    ...
                    && t.occurs_on
                        .iter()
                        .any(|base| ruleset.terrains[base].terrain_type == TerrainType::Land)
            })
            .map(|t| t.name.clone());
        ...
        let mut candidate_plots: Vec<&Tile> = start_tile
            .get_tiles_in_distance_range(1..=range_for_bonuses)
            .filter(|t| {
                t.resource.is_none()
                    && !t
                        .terrain_feature_objects
                        .iter()
                        .any(|f| Some(&f.name) == oasis_equivalent.as_ref())
            })
            .collect();

        candidate_plots.shuffle(&mut rand::thread_rng());
        candidate_plots.sort_by(|a, b| {
            a.aerial_distance_to(start_tile)
                .partial_cmp(&b.aerial_distance_to(start_tile))
                .unwrap()
        });
        ...
        while bonuses_still_needed > 0 && !candidate_plots.is_empty() {
            ...
            if plot.get_base_terrain().has_unique(
                ...
                State::for_testing().with_attacked_tile(Some(plot.clone())),
            ) {
                continue; // Don't put bonuses on snow hills
            }

            let valid_bonuses: Vec<_> = ruleset
                .tile_resources
                .values()
                .filter(|r| {
            ...
            let good_plot_for_oasis = can_place_oasis
                && oasis_equivalent
                    .as_ref()
                    .map_or(false, |o| plot.last_terrain.name == *o);
            ...
            if !valid_bonuses.is_empty() || good_plot_for_oasis {
                ...
                if plot.aerial_distance_to(start_tile) == 1.0 {
                    placed_in_first += 1;
                    if placed_in_first == 2 {
                        // Resort the list in ring order 2,3,1
                        candidate_plots.sort_by(|a, b| {
                            (a.aerial_distance_to(start_tile) * 10.0 - 22.0)
                                .abs()
                                .partial_cmp(
                                    &(b.aerial_distance_to(start_tile) * 10.0 - 22.0).abs(),
                                )
                                .unwrap()
                        });
                    }
                    ...
                    placed_in_second += 1;
                    if placed_in_second == 3 {
                        // Resort the list in ring order 3,1,2
                        candidate_plots.sort_by(|a, b| {
                            (b.aerial_distance_to(start_tile) * 10.0 - 17.0)
                                .abs()
                                .partial_cmp(
                                    &(a.aerial_distance_to(start_tile) * 10.0 - 17.0).abs(),
                                )
                                .unwrap()
                        });
                    }
    ...
    fn get_potential_yield(tile: &Tile, stat: Stat, unimproved: bool) -> f32 {
        ...
        let best_improvement_yield = tile
            .tile_map
            .ruleset
            .as_ref()
            .map(|r| {
                r.tile_improvements
                    .values()
                    .filter(|i| {
                        !i.has_unique(UniqueType::GreatImprovement)
                            && i.unique_to.is_none()
                            && i.terrains_can_be_built_on.contains(&tile.last_terrain.name)
                    })
                    .map(|i| i[stat])
                    .max()
                    .unwrap_or(0.0)
    ...
    fn is_water_only_resource(resource: &TileResource, ruleset: &Ruleset) -> bool {
        resource
            .occurs_on
            .iter()
            .all(|terrain_name| ruleset.terrains[terrain_name].terrain_type == TerrainType::Water)
    }
}
