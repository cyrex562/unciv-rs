...
impl UnitManager {
    ...
    pub fn add_unit_base(&mut self, base_unit: &BaseUnit, city: Option<&City>) -> Option<MapUnit> {
        ...
        let cities_not_in_resistance: Vec<_> = civ_info.cities.iter()
            .filter(|city| !city.is_in_resistance())
            .collect();

        let city_to_add_to = if unit.is_water_unit && (city.is_none() || !city.unwrap().is_coastal()) {
            cities_not_in_resistance.iter()
                .filter(|city| city.is_coastal())
                .choose(&mut rand::thread_rng())
                .or_else(|| civ_info.cities.iter()
                    .filter(|city| city.is_coastal())
                    .choose(&mut rand::thread_rng()))
        ...
        if unit.is_great_person {
            civ_info.add_notification(
                format!("A [{}] has been born in [{}]!", unit.name, city_to_add_to.name),
        ...
        if placed_unit.has_unique(UniqueType::ReligiousUnit) && civ_info.game_info.is_religion_enabled() {
            if !placed_unit.has_unique(UniqueType::TakeReligionOverBirthCity)
                || civ_info.religion_manager.religion.as_ref().map_or(false, |r| !r.is_major_religion()) {
    ...
    pub fn place_unit_near_tile(&mut self, location: (f32, f32), unit_name: &str) -> Option<MapUnit> {
        let civ_info = self.civ_info.as_ref().expect("Civ not set");
        let unit = civ_info.game_info.ruleset.units.get(unit_name)?;
    ...
    ///
    /// created MapUnit or None if no suitable location was found
    pub fn place_unit_near_tile_base(&mut self, location: (f32, f32), base_unit: &BaseUnit, unit_id: Option<i32>) -> Option<MapUnit> {
        let civ_info = self.civ_info.as_ref().expect("Civ not set");
        let unit = civ_info.game_info.tile_map.place_unit_near_tile(location, base_unit, civ_info, unit_id)?;
        ...
        for unique in civ_info.get_triggered_uniques(UniqueType::TriggerUponGainingUnit, &unit.cache.state)
        ...
        for unique in civ_info.get_matching_uniques(UniqueType::LandUnitsCrossTerrainAfterUnitGained, &unit.cache.state) {
        ...
        if unit.has_unique(UniqueType::ReligiousUnit) && civ_info.game_info.is_religion_enabled() {
            unit.religion = civ_info.religion_manager.religion.as_ref().map(|r| r.name.clone());
        }
    ...
    pub fn add_unit_map_unit(&mut self, map_unit: MapUnit, update_civ_info: bool) {
        ...
        let mut new_list = self.get_civ_units_starting_at_next_due()
            .iter()
            .cloned()
            .collect::<Vec<_>>();
