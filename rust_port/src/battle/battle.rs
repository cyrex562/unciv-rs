...
use crate::battle::battle_damage::BattleDamage;
use crate::battle::battle_unit_capture::BattleUnitCapture;
...
impl Battle {
    ...
    pub fn move_preparing_attack(attacker: &dyn ICombatant, attackable_tile: &AttackableTile, try_heal_pillage: bool) -> bool {
        let map_unit_combatant = match attacker.as_any().downcast_ref::<MapUnitCombatant>() {
            Some(combatant) => combatant,
            None => return true,
        };

        let tiles_moved_through = map_unit_combatant.unit.movement.get_distance_to_tiles().get_path_to_tile(&attackable_tile.tile_to_attack_from);
        map_unit_combatant.unit.movement.move_to_tile(&attackable_tile.tile_to_attack_from);
        ...
        if combatant.is_none() || combatant.as_ref().unwrap().get_civ_info() == attacker.get_civ_info() {
            return false;
        }
        ...
        if try_heal_pillage {
            ...
            for tile_to_pillage in tiles_moved_through {
                if map_unit_combatant.unit.current_movement <= 1.0 || map_unit_combatant.unit.health > 90 {
                ...
                if UnitActionsPillage::can_pillage(&map_unit_combatant.unit, &tile_to_pillage)
                    ...
                    if let Some(pillage_action) = UnitActionsPillage::get_pillage_action(&map_unit_combatant.unit, &tile_to_pillage) {
    ...
    pub fn attack_or_nuke(attacker: &dyn ICombatant, attackable_tile: &AttackableTile) -> DamageDealt {
