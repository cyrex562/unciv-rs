...
/// Module for battle unit capture mechanics
pub mod BattleUnitCapture {
    use super::*;

    /// Attempts to capture a military unit after battle
    ///
    ...
    pub fn try_capture_military_unit(
        ...
        let state = StateForConditionals::new(
            ...
            unit: Some(map_unit_defender.unit.clone()),
            our_combatant: Some(defender.clone()),
            their_combatant: Some(attacker.clone()),
            attacked_tile: Some(attacked_tile.clone()),
            combat_action: None,
        );
    ...
    fn unit_captured_prize_ships_unique(
        ...
        if attacker.unit.get_matching_uniques(UniqueType::KillUnitCapture)
            .iter()
            .none(|unique| defender.matches_filter(&unique.params[0])) {
            return false;
        }
    ...
    fn unit_gain_from_defeating_unit(
        ...
        let state = StateForConditionals::new(
            ...
            our_combatant: Some(attacker.clone()),
            their_combatant: Some(defender.clone()),
            attacked_tile: None,
            combat_action: None,
        );
    ...
    fn spawn_captured_unit(
        ...
        if added_unit.get_tile() != defender_tile && civilian_unit.is_some() {
            capture_civilian_unit(attacker, MapUnitCombatant::new(civilian_unit.unwrap()));
        }
    ...
    pub fn capture_civilian_unit(
            ...
            && attacker.get_civ_info().player_type == PlayerType::Human // Only humans get the choice
        {
            ...
            if capture_or_convert_to_worker(captured_unit, attacker.get_civ_info()).is_none() {
                was_destroyed_instead = true;
            }
        }
