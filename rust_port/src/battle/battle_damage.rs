...
/// Module for battle damage calculations
pub mod BattleDamage {
    use super::*;
    use std::cmp::max;
    ...
    fn get_state_for_conditionals(
        ...
        StateForConditionals::new(
            ...
            city: combatant.as_any().downcast_ref::<CityCombatant>().map(|c| c.city.clone()),
            our_combatant: Some(combatant.clone()),
            their_combatant: Some(enemy.clone()),
            attacked_tile: Some(attacked_tile),
            combat_action: Some(combat_action),
        )
    }
    ...
    fn add_unit_unique_modifiers(
        ...
        let strength_malus = adjacent_units.iter()
            ...
            .filter(|u| combatant.matches_filter(&u.params[1]) && combatant.get_tile().matches_filter(&u.params[2]))
            ...
            if map_unit_combatant.is_melee() {
                ...
                if number_of_other_attackers_surrounding_defender > 0 {
                    ...
                    for unique in map_unit_combatant.unit.get_matching_uniques(
                        ...
                        state_for_conditionals: Self::get_state_for_conditionals(CombatAction::Attack, attacker, defender)
                    ) {
    ...
    pub fn get_defence_modifiers(
        ...
        if let Some(map_unit_combatant) = defender.as_any().downcast_ref::<MapUnitCombatant>() {
            if !map_unit_combatant.unit.is_embarked() { // Embarked units get no terrain defensive bonuses
                let tile_defence_bonus = tile.get_defensive_bonus(unit: &map_unit_combatant.unit);

                if (!map_unit_combatant.unit.has_unique(UniqueType::NoDefensiveTerrainBonus, check_civ_info_uniques: true) && tile_defence_bonus > 0
                    || !map_unit_combatant.unit.has_unique(UniqueType::NoDefensiveTerrainPenalty, check_civ_info_uniques: true) && tile_defence_bonus < 0) {
    ...
    fn get_health_dependant_damage_ratio(combatant: &dyn ICombatant) -> f32 {
        if !combatant.as_any().downcast_ref::<MapUnitCombatant>().is_some()
            || combatant.as_any().downcast_ref::<MapUnitCombatant>()
                .map_or(false, |c| c.unit.has_unique(UniqueType::NoDamagePenaltyWoundedUnits, check_civ_info_uniques: true)) {
            return 1.0;
        }
