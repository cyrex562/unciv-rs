...
impl ApiV2Wrapper {
    ...
    pub fn new(base_url: String) -> Self {
        ...
        headers.insert(USER_AGENT, HeaderValue::from_static("Unciv/4.10.14-GNU-Terry-Pratchett"));
    ...
    pub async fn websocket<F, C>(&mut self, handler: F, job_callback: Option<C>) -> Result<bool, UncivNetworkException>
    where
        ...
        C: FnOnce(tokio::task::JoinHandle<()>) + Send + 'static,
    {
        ...
        let url = if self.base_server.starts_with("https://") {
            format!("wss://{}/api/v2/ws", self.base_server.strip_prefix("https://").unwrap_or(&self.base_server))
        } else {
            format!("ws://{}/api/v2/ws", self.base_server.strip_prefix("http://").unwrap_or(&self.base_server))
        };
    ...
    pub async fn version(&self) -> Result<VersionResponse, UncivNetworkException> {
        let response = self.client.get(format!("{}/api/version", this.base_url))
            .send()
            .await
        ...
        if response.status().is_success() {
            ...
            let error_response = response.json::<ApiErrorResponse>()
                .await
            ...
            Err(UncivNetworkException::new(&error_response.message, Some(error_response.status_code)))
        }
    }
