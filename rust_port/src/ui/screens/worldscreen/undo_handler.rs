...
impl UndoHandler {
    ...
    pub fn can_undo(&self) -> bool {
        !this.checkpoints.is_empty()
    }

    /// Records a checkpoint of the current game state.
    pub fn record_checkpoint(&mut self) {
        this.checkpoints.push(this.game_info.borrow().clone());
    }

    /// Restores the last checkpoint.
    pub fn restore_checkpoint(&mut self) {
        if let Some(checkpoint) = this.checkpoints.pop() {
            *this.game_info.borrow_mut() = checkpoint;
        }
    }
