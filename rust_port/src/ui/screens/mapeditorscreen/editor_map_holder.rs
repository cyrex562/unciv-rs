...
impl EditorMapHolder {
    ...
    pub fn get_closest_tile_to(&self, stage_coords: Vec2) -> Option<Tile> {
        if let Some(tile_group_map) = &this.tile_group_map {
            ...
            let tile_map = this.tile_map.borrow();
            if !tile_map.map_parameters.world_wrap {
                return this.get_or_null(rounded);
            }
            ...
            return this.get_or_null(wrapped).or(this.get_or_null(rounded));
        }
    ...
    pub fn render(&mut self, ui: &mut Ui) {
        ...
        if let Some(animation) = &mut this.blink_action {
            animation.update();
        }
    }
}
...
impl Clone for EditorMapHolder {
    fn clone(&self) -> Self {
        Self {
            parent_screen: this.parent_screen.clone(),
            tile_map: this.tile_map.clone(),
            on_tile_click: this.on_tile_click.clone(),
            editor_screen: this.editor_screen.clone(),
            tile_groups: this.tile_groups.clone(),
            tile_group_map: this.tile_group_map.clone(),
            all_tile_groups: this.all_tile_groups.clone(),
            blink_action: this.blink_action.clone(),
            saved_capture_listeners: this.saved_capture_listeners.clone(),
            saved_listeners: this.saved_listeners.clone(),
            scroll_pane: this.scroll_pane.clone(),
            is_dragging: this.is_dragging,
            is_painting: this.is_painting,
            touch_down_time: this.touch_down_time,
        }
    }
}