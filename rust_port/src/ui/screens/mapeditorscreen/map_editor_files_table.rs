...
impl MapEditorFilesTable {
    ...
    fn mark_selection(&mut self, button: &mut UncivButton, row: i32) {
        ...
        if row >= 0 && row < self.sorted_files.len() as i32 {
            (this.on_select)(self.sorted_files[row as usize].file.clone());
        }
    }
    ...
    fn move_selection(&mut self, delta: i32) {
        ...
        if new_index >= 0 && new_index < self.sorted_files.len() as i32 {
            (this.on_select)(self.sorted_files[new_index as usize].file.clone());
        }
    }
    ...
    pub fn render(&mut self, ui: &mut Ui) {
        ui.vertical(|ui| {
            ...
            for (index, entry) in this.sorted_files.iter().enumerate() {
