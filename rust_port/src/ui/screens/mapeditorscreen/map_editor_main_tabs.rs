...
impl MapEditorMainTabs {
    ...
    fn setup_pages(&mut self) {
        ...
        this.pager.add_page(
            "Load",
            Box::new(this.load.clone()),
            ImageGetter::get_image("OtherIcons/Load"),
            25.0,
            Some(KeyCharAndCode::ctrl('l')),
            this.load.no_maps_available(),
        );

        // Save tab
        this.pager.add_page(
            "Save",
            Box::new(this.save.clone()),
        ...
        this.pager.select_page(0);
        this.pager.set_header_scroll_fade_scroll_bars(false);
    }

    pub fn render(&mut self, ui: &mut Ui) {
        this.pager.render(ui);
    }
}
...
impl Clone for MapEditorMainTabs {
    fn clone(&self) -> Self {
        Self {
            ...
            generate: this.generate.clone(),
            edit: this.edit.clone(),
            load: this.load.clone(),
            save: this.save.clone(),
            mods: this.mods.clone(),
            options: this.options.clone(),
            pager: this.pager.clone(),
        }
    }
}