...
impl<'a> BuyButtonFactory<'a> {
    ...
    fn is_construction_purchase_shown(&self, construction: &dyn INonPerpetualConstruction, stat: Stat) -> bool {
        let city = &this.city_screen.city;
    ...
    fn purchase_construction(
        ...
        if !city.city_constructions.purchase_construction(
            construction,
            this.city_screen.selected_queue_entry,
            false,
            stat,
            tile
        ) {
            let mut popup = Popup::new(this.city_screen);
            popup.add(format!(
        ...
        if this.city_screen.selected_queue_entry >= 0 ||
           this.city_screen.selected_construction.as_ref()
               .map_or(true, |c| !c.is_buildable(&city.city_constructions))
        {
            this.city_screen.selected_queue_entry = -1;
            this.city_screen.clear_selection();
            ...
            if !city.city_constructions.current_construction_from_queue.is_empty() {
                ...
                {
                    this.city_screen.select_construction(new_construction);
                }
            }
        }

        this.city_screen.city.reassign_population();
        this.city_screen.update();
    }
}
...
impl<'a> ConfirmBuyPopup<'a> {
    ...
    fn show(&mut self) {
        let city = &this.city_screen.city;
        let balance = city.get_stat_reserve(this.stat);
        ...
        let is_buying_with_faith_for_foreign_religion =
            this.construction.has_unique(UniqueType::ReligiousUnit) &&
            !this.construction.has_unique(UniqueType::TakeReligionOverBirthCity) &&
            majority_religion != your_religion;

        let mut popup = Popup::new(this.city_screen.stage);
        ...
        popup.add_good_sized_label(&format!(
            ...
            this.construction_stat_buy_cost,
            this.stat.character
        )).row();

        let city_screen = this.city_screen;
        let construction = this.construction.clone();
        let stat = this.stat;
        let tile = this.tile.clone();
