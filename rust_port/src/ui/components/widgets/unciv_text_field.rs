...
impl Clone for UncivTextField {
    fn clone(&self) -> Self {
        Self {
            ...
            hide_keyboard: this.hide_keyboard.clone(),
            on_focus_change: this.on_focus_change.clone(),
            focus_listener: UncivTextFieldFocusListener::new(Arc::new(self.clone())),
            visible_area_changed_listener: this.visible_area_changed_listener.as_ref().map(|listener| {
