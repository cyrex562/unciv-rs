...
use crate::models::diplomacy::{DiplomacyFlags, DiplomaticModifiers, DiplomaticStatus, RelationshipLevel};
use crate::models::personality::PersonalityValue;
use crate::models::ruleset::UniqueType;
use crate::models::stats::RankingType;
use crate::models::trade::{TradeLogic, TradeOffer, TradeRequest, TradeOfferType, Trade};
...
impl DiplomacyAutomation {
    ...
    pub fn offer_declaration_of_friendship(civ_info: &mut Civilization) {
        let civs_that_we_can_declare_friendship_with: Vec<_> = civ_info.get_known_civs()
            .iter()
            .filter(|civ| {
                civ_info.diplomacy_functions.can_sign_declaration_of_friendship_with(civ)
                    && !civ_info.get_diplomacy_manager(civ).unwrap().has_flag(DiplomacyFlags::DeclinedDeclarationOfFriendship)
            })
            .sorted_by(|a, b| b.get_diplomacy_manager(civ_info).unwrap().get_relationship_level()
                .partial_cmp(&a.get_diplomacy_manager(civ_info).unwrap().get_relationship_level())
        ...
        for other_civ in civs_that_we_can_declare_friendship_with {
            ...
            let personality_modifier = civ_info.get_personality().modifier_focus(PersonalityValue::Diplomacy, 0.5);
    ...
    pub fn wants_to_sign_declaration_of_friendship(civ_info: &Civilization, other_civ: &Civilization) -> bool {
        ...
        let num_of_friends = civ_info.diplomacy_managers.values()
            .filter(|manager| manager.has_flag(DiplomacyFlags::DeclarationOfFriendship))
            .count();

        let other_civ_number_of_friends = other_civ.diplomacy_managers.values()
            .filter(|manager| manager.has_flag(DiplomacyFlags::DeclarationOfFriendship))
            .count();
        ...
        let all_civs = civ_info.game_info.civilizations.iter()
            .filter(|civ| civ.is_major_civ())
            .count() - 1; // Don't include us

        let dead_civs = civ_info.game_info.civilizations.iter()
            .filter(|civ| civ.is_major_civ() && civ.is_defeated())
            .count();
        ...
        let civs_to_ally_with = 0.25 * all_alive_civs as f32
            * civ_info.get_personality().modifier_focus(PersonalityValue::Diplomacy, 0.3);
        ...
        let war_count = civ_info.diplomacy_managers.values()
            .filter(|manager| {
                let other_civ = civ_info.game_info.get_civilization_by_name(&manager.other_civ_name);
        ...
        let has_non_friendly_neighbors = civ_info.threat_manager.get_neighboring_civilizations()
            .iter()
            .any(|civ| {
                civ.is_major_civ() && civ.name != other_civ.name
                    && civ_info.get_diplomacy_manager(civ).unwrap().is_relationship_level_lt(RelationshipLevel::Favorable)
            });
        ...
        if MotivationToAttackAutomation::has_at_least_motivation_to_attack(civ_info, other_civ, motivation / 2.0) > 0.0 {
            motivation -= 2.0;
        }
    ...
    pub fn offer_open_borders(civ_info: &mut Civilization) {
        ...
        let civs_that_we_can_open_borders_with: Vec<_> = civ_info.get_known_civs()
            .iter()
            .filter(|civ| {
                civ.is_major_civ() && !civ_info.is_at_war_with(civ)
                    && civ.has_unique(UniqueType::EnablesOpenBorders)
                    && !civ_info.get_diplomacy_manager(civ).unwrap().has_open_borders
                    && !civ.get_diplomacy_manager(civ_info).unwrap().has_open_borders
                    && !civ_info.get_diplomacy_manager(civ).unwrap().has_flag(DiplomacyFlags::DeclinedOpenBorders)
        ...
        for other_civ in civs_that_we_can_open_borders_with {
            ...
            if Self::wants_to_open_borders(civ_info, other_civ) {
                ...
                civ_info.get_diplomacy_manager_mut(other_civ).unwrap().set_flag(DiplomacyFlags::DeclinedOpenBorders, 5);
            }
        }
    }
    ...
    pub fn wants_to_open_borders(civ_info: &Civilization, other_civ: &Civilization) -> bool {
        ...
        let has_friends_at_war = civ_info.diplomacy_managers.values().any(|manager| {
            ...
            manager.is_relationship_level_ge(RelationshipLevel::Friend) && other_civ.is_at_war_with(other_civ)
        });
        ...
        let visible_cities = other_civ.cities.iter()
        ...
        if MotivationToAttackAutomation::has_at_least_motivation_to_attack(
                civ_info,
                other_civ,
                diplo_manager.opinion_of_other_civ() * civ_info.get_personality().modifier_focus(PersonalityValue::Commerce, 0.3) / 2.0) > 0.0 {
            return false;
        }
    ...
    pub fn offer_research_agreement(civ_info: &mut Civilization) {
        ...
        let can_sign_research_agreement_civ: Vec<_> = civ_info.get_known_civs()
            .iter()
            .filter(|civ| {
                civ_info.diplomacy_functions.can_sign_research_agreements_with(civ)
                    && !civ_info.get_diplomacy_manager(civ).unwrap().has_flag(DiplomacyFlags::DeclinedResearchAgreement)
                    && !Self::are_we_offering_trade(civ_info, civ, Constants::RESEARCH_AGREEMENT)
            })
            .sorted_by(|a, b| b.stats.stats_for_next_turn.science
        ...
        for other_civ in can_sign_research_agreement_civ {
            ...
            let random_value = rand::thread_rng().gen_range(1..=10);
            let personality_modifier = civ_info.get_personality().modifier_focus(PersonalityValue::Science, 0.3);
            ...
            let mut trade_logic = TradeLogic::new(civ_info, other_civ);
            let cost = civ_info.diplomacy_functions.get_research_agreement_cost(other_civ);
    ...
    pub fn offer_defensive_pact(civ_info: &mut Civilization) {
        ...
        let can_sign_defensive_pact_civ: Vec<_> = civ_info.get_known_civs()
            .iter()
            .filter(|civ| {
                civ_info.diplomacy_functions.can_sign_defensive_pact_with(civ)
                    && !civ_info.get_diplomacy_manager(civ).unwrap().has_flag(DiplomacyFlags::DeclinedDefensivePact)
                    && civ_info.get_diplomacy_manager(civ).unwrap().opinion_of_other_civ() < 70.0 * civ_info.get_personality().inverse_modifier_focus(PersonalityValue::Aggressive, 0.2)
        ...
        for other_civ in can_sign_defensive_pact_civ {
            // Default setting is 3, this will be changed according to different civ.
            let random_value = rand::thread_rng().gen_range(1..=10);
            let personality_modifier = civ_info.get_personality().inverse_modifier_focus(PersonalityValue::Loyal, 0.3);
            ...
            if Self::wants_to_sign_defensive_pact(civ_info, other_civ) {
                ...
                // Remember this for a few turns to save computation power
                civ_info.get_diplomacy_manager_mut(other_civ).unwrap().set_flag(DiplomacyFlags::DeclinedDefensivePact, 5);
            }
        }
    }
    ...
    pub fn wants_to_sign_defensive_pact(civ_info: &Civilization, other_civ: &Civilization) -> bool {
        ...
        let personality_modifier = civ_info.get_personality().inverse_modifier_focus(PersonalityValue::Aggressive, 0.3);
        ...
        for third_civ in common_known_civs {
            ...
            if civ_info.get_diplomacy_manager(third_civ).unwrap().has_flag(DiplomacyFlags::DeclarationOfFriendship)
                && third_civ.get_diplomacy_manager(other_civ).unwrap().is_relationship_level_lt(RelationshipLevel::Favorable) {
                return false;
            }

            // If they have bad relations with any of our friends, don't consider it
            if other_civ.get_diplomacy_manager(third_civ).unwrap().has_flag(DiplomacyFlags::DeclarationOfFriendship)
                && third_civ.get_diplomacy_manager(civ_info).unwrap().is_relationship_level_lt(RelationshipLevel::Neutral) {
                return false;
            }
        }

        let defensive_pacts = civ_info.diplomacy_managers.values()
            .filter(|manager| manager.has_flag(DiplomacyFlags::DefensivePact))
            .count();

        let other_civ_non_overlapping_defensive_pacts = other_civ.diplomacy_managers.values()
            .filter(|manager| {
                manager.has_flag(DiplomacyFlags::DefensivePact)
                    && !manager.other_civ_name.eq(&civ_info.name)
                    && !civ_info.get_diplomacy_manager_by_name(&manager.other_civ_name)
        ...
        let civs_to_ally_with = 0.20 * all_alive_civs as f32
            * civ_info.get_personality().modifier_focus(PersonalityValue::Diplomacy, 0.5);
        ...
        motivation -= (40.0 * (defensive_pacts as f32 - civs_to_ally_with)
            / (all_alive_civs as f32 - civs_to_ally_with)).min(0.0);
    ...
    pub fn declare_war(civ_info: &mut Civilization) {
        ...
        if civ_info.get_personality().get(&PersonalityValue::DeclareWar).unwrap_or(&0.0) == &0.0 {
            return;
        }
        ...
        let our_military_units = civ_info.units.get_civ_units()
            .iter()
        ...
        let total_population: i32 = civ_info.cities.iter()
        ...
        //evaluate war
        let target_civs: Vec<_> = civ_info.get_known_civs()
            .iter()
            .filter(|civ| {
                !civ.is_defeated() && civ.name != civ_info.name && !civ.cities.is_empty()
                    && civ_info.get_diplomacy_manager(civ).unwrap().can_declare_war()
                    && civ.cities.iter().any(|city| civ_info.has_explored(&city.get_center_tile()))
            })
            .collect();
        ...
        let target_civs_with_motivation: Vec<_> = target_civs.iter()
            .map(|civ| (civ, MotivationToAttackAutomation::has_at_least_motivation_to_attack(civ_info, civ, 0.0)))
    ...
    pub fn offer_peace_treaty(civ_info: &mut Civilization) {
        if !civ_info.is_at_war() || civ_info.cities.is_empty() || civ_info.diplomacy_managers.is_empty() {
            return;
        }

        let enemies_civ: Vec<_> = civ_info.diplomacy_managers.values()
            .filter(|manager| manager.get_diplomatic_status() == DiplomaticStatus::War)
            .map(|manager| civ_info.game_info.get_civilization_by_name(&manager.other_civ_name))
            .filter(|civ| {
                civ.name != civ_info.name && !civ.is_barbarian && !civ.cities.is_empty()
                    && !civ.get_diplomacy_manager(civ_info).unwrap().has_flag(DiplomacyFlags::DeclaredWar)
                    && !civ_info.get_diplomacy_manager(civ).unwrap().has_flag(DiplomacyFlags::DeclaredWar)
                    && !civ_info.get_diplomacy_manager(civ).unwrap().has_flag(DiplomacyFlags::DeclinedPeace)
            })
            .filter(|civ| {
                // Don't allow AIs to offer peace to city states allied with their enemies
                !(civ.is_city_state && civ.get_ally_civ().is_some()
                    && civ_info.is_at_war_with(civ_info.game_info.get_civilization_by_name(civ.get_ally_civ().unwrap())))
            })
            .filter(|civ| {
        ...
        for enemy in enemies_civ {
            if MotivationToAttackAutomation::has_at_least_motivation_to_attack(civ_info, enemy, 10.0) >= 10.0 {
                // We can still fight. Refuse peace.
                continue;
            }

            if civ_info.get_stat_for_ranking(RankingType::Force) - 0.8 * civ_info.threat_manager.get_combined_force_of_warring_civs() > 0.0 {
                let random_seed = civ_info.game_info.civilizations.iter().position(|c| c.name == enemy.name).unwrap_or(0)
                    + civ_info.get_civs_at_war_with().len() + 123 * civ_info.game_info.turns;
            ...
            if enemy.is_major_civ() {
                let mut money_we_need_to_pay = -TradeEvaluation::evaluate_peace_cost_for_them(civ_info, enemy);
    ...
    pub fn ask_for_help(civ_info: &mut Civilization) {
        ...
        let enemy_civs: Vec<_> = civ_info.get_civs_at_war_with()
            .iter()
            .filter(|civ| civ.is_major_civ())
        ...
        for enemy_civ in enemy_civs {
            let potential_allies: Vec<_> = enemy_civ.threat_manager.get_neighboring_civilizations()
                .iter()
                .filter(|civ| {
                    civ_info.knows(civ) && !civ.is_at_war_with(enemy_civ)
                        && civ_info.get_diplomacy_manager(civ).unwrap().is_relationship_level_ge(RelationshipLevel::Friend)
                        && !civ.get_diplomacy_manager(civ_info).unwrap().has_flag(DiplomacyFlags::DeclinedJoinWarOffer)
                })
                .sorted_by(|a, b| b.get_stat_for_ranking(RankingType::Force)
                    .partial_cmp(&a.get_stat_for_ranking(RankingType::Force))
                    .unwrap_or(std::cmp::Ordering::Equal))
            ...
            let civ_to_ask = potential_allies.iter()
                .find(|civ| DeclareWarPlanEvaluator::evaluate_join_our_war_plan(civ_info, enemy_civ, civ, None) > 0.0);
    ...
    fn are_we_offering_trade(civ_info: &Civilization, other_civ: &Civilization, offer_name: &str) -> bool {
        other_civ.trade_requests.iter()
            .filter(|request| request.from_civ == civ_info.name)
            .any(|trade| {
                trade.trade.our_offers.iter().any(|offer| offer.target == offer_name)
                    || trade.trade.their_offers.iter().any(|offer| offer.target == offer_name)
            })
    }
}