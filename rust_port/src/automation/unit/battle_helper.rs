...
impl BattleHelper {
    ...
    pub fn try_attack_nearby_enemy(unit: &mut MapUnit, stay_on_tile: bool) -> bool {
        ...
        let attackable_enemies: Vec<_> = TargetHelper::get_attackable_enemies(unit, &distance_to_tiles, stay_on_tile)
            .iter()
            .filter(|enemy| {
    ...
    pub fn try_disembark_unit_to_attack_position(unit: &mut MapUnit) -> bool {
        ...
        let attackable_enemies_next_turn: Vec<_> = TargetHelper::get_attackable_enemies(unit, &unit_distance_to_tiles)
            .iter()
            .filter(|enemy| {
                BattleDamage::calculate_damage_to_attacker(
                    MapUnitCombatant::new(unit),
    ...
    fn get_city_attack_value(attacker: &MapUnit, city: &City) -> i32 {
        ...
        let is_city_capturable = city.health == 1.0
            || (attacker.base_unit.is_melee() && city.health <= BattleDamage::calculate_damage_to_defender(&attacker_unit, &city_unit).max(1.0));
        ...
        if attacker.base_unit.is_melee() {
            let battle_damage = BattleDamage::calculate_damage_to_attacker(&attacker_unit, &city_unit);
    ...
    fn get_unit_attack_value(attacker: &MapUnit, attack_tile: &AttackableTile) -> i32 {
        ...
        if let Some(military_unit) = &attack_tile.tile_to_attack.military_unit {
            ...
            let attacks_to_kill = (military_unit.health as f64 /
            ...
            if attacks_to_kill <= 1.0 {
                attack_value += 30;
            }
            // On average, this should take around 3 turns, so -15
            else {
                attack_value -= (attacks_to_kill * 5.0) as i32;
            }
        ...
        attack_value += (attack_tile.movement_left_after_moving_to_attack_tile * 5) as i32;
