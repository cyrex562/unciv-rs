...
use crate::models::{Belief, BeliefType, Counter, Religion};
use crate::ui::screens::worldscreen::unit::actions::UnitActionModifiers;
use crate::utils::extensions::{FillPlaceholders, ToPercent};
...
impl ReligionManager {
    ...
    pub fn using_free_beliefs(&self) -> bool {
        (self.religion_state == ReligionState::None && self.stored_faith < self.faith_for_pantheon()) // first pantheon is free
            ...
            || (self.religion_state == ReligionState::Religion || self.religion_state == ReligionState::EnhancedReligion)
        // any belief adding outside of great prophet use
    }

    pub fn faith_for_pantheon(&self, additional_civs: i32) -> i32 {
        let civ = self.civ.as_ref().expect("Civ not set");
        let game_info = &civ.game_info;
        let num_civs = additional_civs
            + game_info
                .civilizations
                .iter()
    ...
    pub fn can_found_or_expand_pantheon(&self) -> bool {
        let civ = self.civ.as_ref().expect("Civ not set");
        if !civ.game_info.is_religion_enabled() {
            return false;
        }
        if self.religion_state > ReligionState::Pantheon {
            return false;
        }
        if !civ.is_major_civ() {
            return false;
        ...
        if civ
            ...
            .civilizations
            .iter()
            .any(|c| c.religion_manager.religion_state == ReligionState::EnhancedReligion)
            && civ
                .game_info
                .civilizations
                .iter()
    ...
    fn found_pantheon(&mut self, belief_name: String, use_free_belief: bool) {
        ...
        civ.game_info
            .religions
            .insert(belief_name.clone(), religion.clone());
        for city in &civ.cities {
            city.religion
                .add_pressure(belief_name.clone(), 200 * city.population.population);
        }
    ...
    pub fn great_prophets_earned(&self) -> i32 {
        let civ = self.civ.as_ref().expect("Civ not set");
        let prophet_name = self
            .get_great_prophet_equivalent()
            .map(|u| u.name.clone())
            .unwrap_or_default();
        civ.civ_constructions
    ...
    pub fn faith_for_next_great_prophet(&self) -> i32 {
        let civ = self.civ.as_ref().expect("Civ not set");
        let great_prophets_earned = self.great_prophets_earned();

        let mut faith_cost = (200.0
            + 100.0 * great_prophets_earned as f32 * (great_prophets_earned + 1) as f32 / 2.0)
    ...
    pub fn can_generate_prophet(&self, ignore_faith_amount: bool) -> bool {
        ...
        if !civ.game_info.is_religion_enabled() {
            return false;
        } // No religion, no prophets
        if self.religion.is_none() || self.religion_state == ReligionState::None {
            return false;
        } // First get a pantheon, then we'll talk about a real religion
        if self.get_great_prophet_equivalent().is_none() {
            return false;
        }
        if !ignore_faith_amount && self.stored_faith < self.faith_for_next_great_prophet() {
            return false;
        }
        if !civ.is_major_civ() {
            return false;
        }
        if civ.has_unique(UniqueType::MayNotGenerateGreatProphet) {
            return false;
        }
        if self.religion_state == ReligionState::Pantheon
            && self.remaining_foundable_religions() == 0
        {
            return false;
        } // too many have been founded
        true
    }
    ...
    fn generate_prophet(&mut self) {
        ...
        let prophet_spawn_change =
            (5.0 + self.stored_faith as f32 - self.faith_for_next_great_prophet() as f32) / 100.0;
        ...
        if Random::new(civ.game_info.turns).next_f32() < prophet_spawn_change {
            ...
            self.stored_faith -= self.faith_for_next_great_prophet();
            civ.civ_constructions
                .bought_items_with_increasing_price
                .add(prophet_unit.name.clone(), 1);
        }
    }
    ...
    fn max_number_of_religions(&self) -> usize {
        ...
        let civ_count = game_info
            .civilizations
            .iter()
            .filter(|c| c.is_major_civ())
            .count();
        std::cmp::min(
            ruleset.religions.len(),
            base + (civ_count as f32 * multiplier) as usize,
        )
    }
    ...
    fn founded_religions_count(&self) -> usize {
        ...
        civ.game_info
            ...
            .iter()
            .filter(|c| {
                c.religion_manager.religion.is_some()
    ...
    pub fn remaining_foundable_religions(&self) -> i32 {
        ...
        let max_number_of_additional_religions =
            self.max_number_of_religions() - self.founded_religions_count();

        let available_beliefs_to_found = std::cmp::min(
            self.number_of_beliefs_available(BeliefType::Follower),
            self.number_of_beliefs_available(BeliefType::Founder),
        );

        std::cmp::min(
            max_number_of_additional_religions as i32,
            available_beliefs_to_found,
        )
    }
    ...
    pub fn remaining_foundable_religions_breakdown(&self) -> Vec<(String, i32)> {
        ...
        breakdown.push((
            "Available religion symbols".to_string(),
            ruleset.religions.len() as i32,
        ));
        ...
        if hide_civ_count {
            let known_civs = 1 + civ
                .get_known_civs()
                .iter()
                .filter(|c| c.is_major_civ())
                .count();
            let estimated_civ_count = (game_info
                .game_parameters
                .min_number_of_players
                .max(known_civs)
                + game_info.game_parameters.max_number_of_players
                - 1)
                / 2
                + 1;
            ...
            breakdown.push((
                format!("Number of civilizations * [{}] + [{}]", multiplier, base),
                civs_and_base,
            ));
        }

        breakdown.push((
            "Religions already founded".to_string(),
            self.founded_religions_count() as i32,
        ));
        breakdown.push((
            "Available founder beliefs".to_string(),
            self.number_of_beliefs_available(BeliefType::Founder),
        ));
        breakdown.push((
            "Available follower beliefs".to_string(),
            self.number_of_beliefs_available(BeliefType::Follower),
        ));
    ...
    pub fn may_found_religion_at_all(&self) -> bool {
        ...
        if self.religion_state >= ReligionState::Religion {
            return false;
        } // Already created a major religion
        if !civ.is_major_civ() {
            return false;
        } // Only major civs may use religion
        if self.remaining_foundable_religions() == 0 {
            return false;
        } // Too bad, too many religions have already been founded
        true
    }

    pub fn may_found_religion_here(&self, tile: &Tile) -> bool {
        if !self.may_found_religion_at_all() {
            return false;
        }
        if !tile.is_city_center() {
            return false;
        }
        if tile.get_city().map_or(false, |c| c.is_holy_city()) {
            return false;
        }
    ...
    pub fn found_religion(&mut self, prophet: &MapUnit) {
        if !self.may_found_religion_here(prophet.get_tile()) {
            return;
        } // How did you do this?
        if self.religion_state == ReligionState::None {
    ...
    pub fn may_enhance_religion_at_all(&self) -> bool {
        ...
        if self.religion.is_none() {
            return false;
        } // First found a pantheon
        if self.religion_state != ReligionState::Religion {
            return false;
        } // First found an actual religion
        if !civ.is_major_civ() {
            return false;
        } // Only major civs
        if self.number_of_beliefs_available(BeliefType::Follower) == 0 {
            return false;
        } // Mod maker did not provide enough follower beliefs
        if self.number_of_beliefs_available(BeliefType::Enhancer) == 0 {
            return false;
        } // Mod maker did not provide enough enhancer beliefs
        true
    }

    pub fn may_enhance_religion_here(&self, tile: &Tile) -> bool {
        if !self.may_enhance_religion_at_all() {
            return false;
        }
        if !tile.is_city_center() {
            return false;
    ...
    pub fn use_prophet_for_enhancing_religion(&mut self, prophet: &MapUnit) {
        if !self.may_enhance_religion_here(prophet.get_tile()) {
            return;
        } // How did you do this?
    ...
    fn get_beliefs_to_choose_at_prophet_use(
        &self,
        enhancing_religion: bool,
    ) -> Counter<BeliefType> {
        let civ = self.civ.as_ref().expect("Civ not set");
        let action = if enhancing_religion {
            "enhancing"
        } else {
            "founding"
        };
        ...
        for belief_type in BeliefType::iter() {
            ...
            available_beliefs.add(belief_type, self.number_of_beliefs_available(belief_type));
        }
        ...
        for (belief_type, count) in self.free_beliefs_as_enums().entries() {
    ...
    pub fn get_beliefs_to_choose_at_founding(&self) -> Counter<BeliefType> {
        self.get_beliefs_to_choose_at_prophet_use(false)
    }

    pub fn get_beliefs_to_choose_at_enhancing(&self) -> Counter<BeliefType> {
        self.get_beliefs_to_choose_at_prophet_use(true)
    }
    ...
    pub fn choose_beliefs(&mut self, beliefs: Vec<Belief>, use_free_beliefs: bool) {
        ...
        if self.religion_state == ReligionState::None {
            self.found_pantheon(beliefs[0].name.clone(), use_free_beliefs); // makes religion non-null
        }
        // add beliefs (religion exists at this point)
        self.religion.as_mut().unwrap().add_beliefs(beliefs.clone());

        match self.religion_state {
            ReligionState::None => {
        ...
        for unique in civ.get_triggered_uniques(UniqueType::TriggerUponAdoptingPolicyOrBelief) {
            for belief in &beliefs {
                if unique
                    .get_modifiers(UniqueType::TriggerUponAdoptingPolicyOrBelief)
                    .iter()
    ...
    pub fn found_religion_with_name(&mut self, display_name: String, name: String) {
        let civ = self.civ.as_ref().expect("Civ not set");
        let mut new_religion =
            Religion::new(name.clone(), civ.game_info.clone(), civ.civ_name.clone());
        ...
        if let Some(holy_city) = civ.cities.iter().find(|c| c.id == self.founding_city_id) {
            ...
            holy_city
                .religion
                .add_pressure(name, holy_city.population.population * 500);
        }
        ...
        for unit in civ.units.get_civ_units() {
            if unit.has_unique(UniqueType::ReligiousUnit)
                && unit.has_unique(UniqueType::TakeReligionOverBirthCity)
            {
    ...
    pub fn may_spread_religion_at_all(&self, missionary: &MapUnit) -> bool {
        ...
        if !civ.game_info.is_religion_enabled() {
            return false;
        } // No religion, no spreading

        let religion = missionary
            .civ
            .game_info
            .religions
            .get(&missionary.religion?)?;
        if religion.is_pantheon() {
            return false;
        }
        if UnitActionModifiers::get_usable_unit_action_uniques(
            missionary,
    ...
    pub fn may_spread_religion_now(&self, missionary: &MapUnit) -> bool {
        if !self.may_spread_religion_at_all(missionary) {
            return false;
        }
        if missionary.get_tile().get_owner().is_none() {
            return false;
        }
        if missionary
            .current_tile
        ...
        if missionary.get_tile().get_city().map_or(false, |c| {
            c.religion.is_protected_by_inquisitor(&missionary.religion?)
        }) {
            return false;
        }
        true
    ...
    pub fn number_of_cities_following_this_religion(&self) -> i32 {
        ...
        civ.game_info
            ...
            .filter(|c| {
                c.religion
                    .get_majority_religion()
                    .map_or(false, |r| r == *self.religion.as_ref().unwrap())
            })
            .count() as i32
    }
    ...
    pub fn number_of_followers_following_this_religion(&self, city_filter: &str) -> i32 {
        ...
        civ.game_info
            ...
            .map(|c| {
                c.religion
                    .get_followers_of(self.religion.as_ref().unwrap().name.clone())
            })
            .sum()
    }
    ...
    pub fn get_majority_religion(&self) -> Option<Religion> {
        ...
        let max_religion_counter_entry =
            religion_counter.entries().max_by_key(|(_, count)| *count)?;
