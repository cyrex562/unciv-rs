...
use crate::models::ruleset::unique::{
    StateForConditionals, UniqueMap, UniqueTriggerActivation, UniqueType,
};
...
impl PolicyManager {
    ...
    pub fn set_transients(&mut self, civ_info: Arc<Civilization>) {
        ...
        for policy_name in &self.adopted_policies {
    ...
    fn remove_policy_from_transients(&mut self, policy: &Policy) {
        for unique in &policy.unique_objects {
            self.policy_uniques.remove_unique(unique.clone());
        }
    }
    ...
    pub fn add_culture(&mut self, culture: i32) {
        ...
        self.stored_culture += culture;
        if !could_adopt_policy_before && self.can_adopt_policy() {
            self.should_open_policy_picker = true;
        }
    }
    ...
    pub fn get_culture_refund_map(
        ...
        for policy in policies_to_remove {
            policy_cost_input -= 1;
            let refund = (self.get_policy_culture_cost(policy_cost_input) as f64
                * refund_percentage as f64
                / 100.0) as i32;
    ...
    pub fn get_policy_culture_cost(&self, number_of_adopted_policies: i32) -> i32 {
        ...
        let world_size_modifier = civ_info
            .game_info
            .tile_map
            .map_parameters
            .map_size
            .get_predefined_or_next_smaller()
            .policy_cost_per_city_modifier;
        let mut city_modifier = world_size_modifier
            * (civ_info.cities.iter().filter(|c| !c.is_puppet).count() - 1) as f64;
    ...
    pub fn is_adoptable(&self, policy: &Policy, check_era: bool) -> bool {
        ...
        if !policy
            .requires
            .as_ref()
            .map_or(false, |reqs| reqs.iter().all(|req| self.is_adopted(req)))
        {
            return false;
        }
        ...
        if check_era {
            ...
            let era = civ_info
                ...
                .ruleset
                .eras
                .get(&policy.branch.era)
                .expect("Era not found");
    ...
    pub fn can_adopt_policy(&self) -> bool {
        ...
        if self.free_policies == 0
            && self.stored_culture < self.get_culture_needed_for_next_policy()
        {
            return false;
        }
    ...
    pub fn adopt(&mut self, policy: &Policy, branch_completion: bool) {
        ...
        if !branch_completion {
            if self.free_policies > 0 {
                ...
                self.stored_culture -= culture_needed_for_next_policy;
                self.number_of_adopted_policies += 1;
            }
        }

        self.adopted_policies.insert(policy.name.clone());
        ...
        if !branch_completion {
            ...
            if branch
                .policies
                .iter()
                .filter(|p| self.is_adopted(&p.name))
                .count()
                == branch.policies.len() - 1
            {
        ...
        for unique in &policy.unique_objects {
            if !unique.has_trigger_conditional() && unique.conditionals_apply(&civ_info.state) {
                UniqueTriggerActivation::trigger_unique(
                    unique,
                    civ_info.clone(),
                    Some(&trigger_notification_text),
                );
            }
        }

        for unique in civ_info
            .get_triggered_uniques(UniqueType::TriggerUponAdoptingPolicyOrBelief)
            .iter()
        ...
        if !self.can_adopt_policy() {
            self.should_open_policy_picker = false;
        }
    }
    ...
    pub fn remove_policy(
        &mut self,
        policy: &Policy,
        branch_completion: bool,
        assume_was_free: bool,
    ) {
        let civ_info = self.civ_info.as_ref().expect("CivInfo not set");

        if !self.adopted_policies.remove(&policy.name) {
            panic!("Attempt to remove non-adopted Policy {}", policy.name);
        }

        if !assume_was_free && self.number_of_adopted_policies > 0 {
            self.number_of_adopted_policies -= 1;
        }
            ...
            .filter(|c| c.is_major_civ())
        {
            ...
            let default_notification_text = if civ.get_known_civs().contains(civ_info) {
                format!(
                    ...
                    civ_info.civ_name, policy.name
                )
            } else {
                format!(
                    "An unknown civilization has adopted the [{}] policy",
                    policy.name
                )
            };
            ...
            civ.add_notification(
                &format!(
                    "{{{}}} {{{}}}",
                    default_notification_text, extra_notification_text
                ),
    ...
    pub fn get_culture_from_great_writer(&self) -> i32 {
        let civ_info = self.civ_info.as_ref().expect("CivInfo not set");
        (self.culture_of_last_8_turns.iter().sum::<i32>() as f64
            * civ_info.game_info.speed.culture_cost_modifier) as i32
    }
    ...
    fn add_current_culture_to_culture_of_last_8_turns(&mut self, culture: i32) {
        let civ_info = self.civ_info.as_ref().expect("CivInfo not set");
        self.culture_of_last_8_turns[civ_info.game_info.turns as usize % 8] = culture;
    }

    /// Checks if all policies are adopted
    pub fn all_policies_adopted(&self, check_era: bool) -> bool {
        !self
            .get_ruleset_policies()
            .values()
            .any(|p| self.is_adoptable(p, check_era))
    }
    ...
    pub fn get_priority_map(&self) -> HashMap<PolicyBranch, i32> {
        ...
        for branch in self.branches.iter() {
            let victory_priority = civ_info
                .get_preferred_victory_types()
                .iter()
                .map(|vt| branch.priorities.get(vt).unwrap_or(&0))
                .sum::<i32>();

            let personality_priority = civ_info
                .get_personality()
                .priorities
                .get(&branch.name)
                .unwrap_or(&0);
    ...
    pub fn should_show_policy_picker(&self) -> bool {
        (self.should_open_policy_picker || self.free_policies > 0) && self.can_adopt_policy()
    }
}
