...
fn remove_missing_great_person_points(game_info: &mut GameInfo) {
    for civ in &mut game_info.civilizations {
        ...
        let great_general_keys: Vec<String> = civ
            .great_people
            .great_general_points_counter
            .keys()
            .cloned()
            .collect();
        ...
        let great_person_keys: Vec<String> = civ
            .great_people
            .great_person_points_counter
            .keys()
            .cloned()
            .collect();
...
impl GreatPersonManager {
    ...
    fn get_pool_key(&self, great_person: &str) -> String {
        ...
        civ_info
            .get_equivalent_unit(great_person)
    ...
    pub fn get_points_required_for_great_person(&self, great_person: &str) -> i32 {
        ...
        let points = self
            .points_for_next_great_person_counter
            .get(&key)
            .unwrap_or(&0);
        if *points == 0 {
            // Initialize with default value if not set
            self.points_for_next_great_person_counter
                .insert(key.clone(), 100);
    ...
    pub fn get_new_great_person(&mut self) -> Option<String> {
        ...
        for (unit, value) in self.great_general_points_counter.iter() {
            ...
            let required_points = if *required_points == 0 {
                self.points_for_next_great_general_counter
                    .insert(unit.clone(), 200);
                200
            } else {
                *required_points
            };
            ...
            if *value > required_points {
                self.great_general_points_counter
                    .add(unit, -required_points);
        ...
        for (great_person, value) in self.great_person_points_counter.iter() {
            ...
            if *value >= required_points {
                self.great_person_points_counter
                    .add(great_person, -required_points);
                ...
                let current_points = self
                    ...
                    .unwrap_or(&0);
                self.points_for_next_great_person_counter
                    .insert(pool_key, current_points * 2);
    ...
    pub fn add_great_person_points(&mut self) {
        self.great_person_points_counter
            .add_all(self.get_great_person_points_for_next_turn());
    }
    ...
    pub fn trigger_mayan_great_person(&mut self) {
        ...
        if self.long_count_gp_pool.is_empty() {
            self.long_count_gp_pool = great_people.iter().map(|gp| gp.name.clone()).collect();
        }

        self.free_great_people += 1;
        self.maya_limited_free_gp += 1;
