use crate::civilization::{
    Civilization, NotificationCategory, NotificationIcon, PlayerType, Proximity,
};
...
impl QuestManager {
    ...
    pub fn set_transients(&mut self, civ: Arc<Civilization>) {
        ...
        for quest in &mut self.assigned_quests {
    ...
    pub fn have_quests_for(&self, challenger: &Civilization) -> bool {
        self.get_assigned_quests_for(&challenger.civ_name)
            .next()
            .is_some()
    }

    /// Gets all assigned quests for a civilization
    pub fn get_assigned_quests_for<'a>(
        &'a self,
        civ_name: &str,
    ) -> impl Iterator<Item = &'a AssignedQuest> {
        self.assigned_quests
            .iter()
            .filter(move |q| q.assignee == *civ_name)
    }

    /// Gets all assigned quests of a specific type
    fn get_assigned_quests_of_name<'a>(
        &'a self,
        quest_name: QuestName,
    ) -> impl Iterator<Item = &'a AssignedQuest> {
        self.assigned_quests
            .iter()
            .filter(move |q| q.quest_name_instance() == quest_name)
    }
    ...
    pub fn get_investment_multiplier(&self, donor: &str) -> f64 {
        let investment_quest = self
            .get_assigned_quests_of_name(QuestName::Invest)
    ...
    pub fn clone(&self) -> Self {
        ...
        to_return.global_quest_countdown = self.global_quest_countdown;
        to_return.individual_quest_countdown = self.individual_quest_countdown.clone();
        to_return.assigned_quests = self.assigned_quests.clone();
        to_return.units_to_kill_for_civ = self.units_to_kill_for_civ.clone();

        for (attacker, units_killed) in &self.units_killed_from_civ {
            to_return
                .units_killed_from_civ
                .insert(attacker.clone(), units_killed.clone());
        }
    ...
    pub fn end_turn(&mut self) {
        ...
        if civ.is_defeated() {
            self.assigned_quests.clear();
            self.individual_quest_countdown.clear();
            self.global_quest_countdown = UNSET;
            return;
        }
    ...
    fn decrement_quest_countdowns(&mut self) {
        if self.global_quest_countdown > 0 {
            self.global_quest_countdown -= 1;
        }

        for (_, countdown) in self.individual_quest_countdown.iter_mut() {
            if *countdown > 0 {
                *countdown -= 1;
            }
        }
    ...
    fn seed_global_quest_countdown(&mut self) {
        ...
        if self.global_quest_countdown != UNSET {
            return;
        }
        ...
        let countdown = if civ.game_info.turns == GLOBAL_QUEST_FIRST_POSSIBLE_TURN {
            ...
            GLOBAL_QUEST_MIN_TURNS_BETWEEN
                + (rand::random::<i32>() % GLOBAL_QUEST_RAND_TURNS_BETWEEN)
        };

        self.global_quest_countdown = (countdown as f64 * civ.game_info.speed.modifier) as i32;
    }
    ...
    fn seed_individual_quests_countdowns(&mut self) {
        ...
        for major_civ in major_civs {
            if !self
                .individual_quest_countdown
                .contains_key(&major_civ.civ_name)
                || self.individual_quest_countdown[&major_civ.civ_name] == UNSET
            {
    ...
    fn seed_individual_quests_countdown(&mut self, challenger: &Civilization) {
        ...
        let countdown = if civ.game_info.turns == INDIVIDUAL_QUEST_FIRST_POSSIBLE_TURN {
            ...
            INDIVIDUAL_QUEST_MIN_TURNS_BETWEEN
                + (rand::random::<i32>() % INDIVIDUAL_QUEST_RAND_TURNS_BETWEEN)
        };

        self.individual_quest_countdown.insert(
    ...
    fn try_start_new_global_quest(&mut self) {
        ...
        if self
            ...
            .count()
            >= GLOBAL_QUEST_MAX_ACTIVE as usize
        {
            return;
        }
    ...
    fn try_start_new_individual_quests(&mut self) {
        ...
        for (challenger_name, countdown) in &self.individual_quest_countdown {
            ...
            if self
                .get_assigned_quests_for(&challenger.civ_name)
                .filter(|q| q.is_individual())
                .count()
                >= INDIVIDUAL_QUEST_MAX_ACTIVE as usize
            {
                continue;
            }

            let assignable_quests =
                self.get_quests(|q| q.is_individual() && self.is_quest_valid(q, &challenger));
    ...
    fn try_barbarian_invasion(&mut self) {
        ...
        if (civ.get_turns_till_call_for_barb_help().is_none()
            || civ.get_turns_till_call_for_barb_help() == 0)
    ...
    fn handle_global_quests(&mut self) {
        ...
        self.assigned_quests.retain(|q| {
            !q.is_global()
                || self.can_assign_a_quest_to(&civ.game_info.get_civilization(&q.assignee))
        });

        let global_quests_expired = self
            .assigned_quests
            .iter()
    ...
    fn handle_global_quest(&mut self, quest_name: QuestName) {
        ...
        self.assigned_quests
            .retain(|q| q.quest_name_instance() != quest_name);
    }

    /// Handles individual quests
    fn handle_individual_quests(&mut self) {
        self.assigned_quests
            .retain(|q| !q.is_individual() || !self.handle_individual_quest(q));
    }
        ...
        for assignee in assignees {
            ...
            match quest.quest_name_instance {
                ...
                QuestName::ConnectResource => {
                    data1 = self
                        .get_resource_for_quest(&assignee)
                        .expect("No resource found")
                        .name;
                }
                QuestName::ConstructWonder => {
                    data1 = self
                        .get_wonder_to_build_for_quest(&assignee)
                        .expect("No wonder found")
                        .name;
                }
                QuestName::GreatPerson => {
                    data1 = self
                        .get_great_person_for_quest(&assignee)
                        .expect("No great person found")
                        .name;
                }
                QuestName::FindPlayer => {
                    data1 = self
                        .get_civilization_to_find_for_quest(&assignee)
                        .expect("No civilization found")
                        .civ_name;
                }
                QuestName::FindNaturalWonder => {
                    data1 = self
                        .get_natural_wonder_to_find_for_quest(&assignee)
                        .expect("No natural wonder found");
                }
                QuestName::ConquerCityState => {
                    data1 = self
                        .get_city_state_target(&assignee)
                        .expect("No city state target found")
                        .civ_name;
                }
            ...
            self.assigned_quests.push(new_quest);

            if quest.is_individual() {
                self.individual_quest_countdown
                    .insert(assignee.civ_name.clone(), UNSET);
            }

            assignee.add_notification(
                &format!(
                    "[{}] assigned you a new quest: [{}].",
                    civ.civ_name, quest.name
                ),
    ...
    fn can_assign_a_quest_to(&self, challenger: &Civilization) -> bool {
        ...
        !challenger.is_defeated()
            && challenger.is_major_civ()
            && civ.knows(challenger)
            && !civ.is_at_war_with(challenger)
    }
    ...
    fn is_quest_valid(&self, quest: &Quest, challenger: &Civilization) -> bool {
        ...
        if self
            .get_assigned_quests_of_name(quest.quest_name_instance)
        ...
        if quest.is_individual()
            && civ
                .get_diplomacy_manager(&challenger.civ_name)
        ...
        match quest.quest_name_instance {
            ...
            QuestName::FindPlayer => self
                .get_civilization_to_find_for_quest(challenger)
                .is_some(),
            QuestName::FindNaturalWonder => self
                .get_natural_wonder_to_find_for_quest(challenger)
                .is_some(),
            QuestName::PledgeToProtect => {
    ...
    fn is_denounce_civ_quest_valid(
        &self,
        challenger: &Civilization,
        most_recent_bully: Option<&str>,
    ) -> bool {
        let civ = self.civ.as_ref().expect("Civ not set");
        ...
        if let Some(bully_name) = most_recent_bully {
            challenger.knows(bully_name)
                && !challenger
                    .get_diplomacy_manager(bully_name)
    ...
    fn is_quest_completed(&self, quest: &AssignedQuest) -> bool {
        ...
        match quest.quest_name_instance {
            QuestName::ClearBarbarianCamp => {
                ...
                !civ.game_info
                    .get_tile(&position)
                    .map_or(false, |t| t.is_barbarian_camp())
            }
            QuestName::Route => assignee.is_capital_connected_to_city(civ.get_capital()?),
            QuestName::ConnectResource => {
                let resource_name = &quest.data1;
                civ.get_capital()?
                    .get_tile()
                    .get_resource()
                    .map_or(false, |r| r.name == *resource_name)
            }
            QuestName::ConstructWonder => {
            ...
            QuestName::GreatPerson => {
                ...
                civ.get_great_people()
                    .iter()
                    .any(|gp| gp.name == *great_person_name)
            }
            QuestName::FindPlayer => {
            ...
            QuestName::FindNaturalWonder => {
                let wonder_name = &quest.data1;
                civ.get_explored_tiles()
                    .iter()
                    .any(|t| t.is_natural_wonder() && t.get_natural_wonder_name() == *wonder_name)
            }
            QuestName::PledgeToProtect => {
                let target_name = &quest.data1;
                civ.city_state_functions
                    .get_protector_civs()
                    .contains(&assignee.civ_name)
            }
            QuestName::GiveGold => {
                let target_name = &quest.data1;
                assignee.gold >= 250
            ...
            QuestName::DenounceCiv => {
                let target_name = &quest.data1;
                assignee
                    .get_diplomacy_manager(target_name)
                    .map_or(false, |dm| dm.has_flag(DiplomacyFlags::Denunciation))
            }
            QuestName::SpreadReligion => {
            ...
            QuestName::ConquerCityState => {
                let target_name = &quest.data1;
                !civ.game_info
                    .get_civilization(target_name)
                    .map_or(false, |cs| cs.is_city_state())
            }
            QuestName::BullyCityState => {
                let target_name = &quest.data1;
    ...
    fn award_quest_completion(&mut self, quest: &AssignedQuest) {
        ...
        if reward > 0 {
            ...
            assignee.add_notification(
                &format!(
                    "You completed a quest for [{}] and received {} influence!",
                    civ.civ_name, reward
                ),
                vec![NotificationAction::Diplomacy(civ.civ_name.clone())],
                NotificationCategory::Diplomacy,
            ...
            civ.add_notification(
                &format!(
                    "[{}] completed your quest and received {} influence!",
                    assignee.civ_name, reward
                ),
    ...
    fn handle_quest_expiration(&mut self, quest: &AssignedQuest) {
        ...
        assignee.add_notification(
            &format!(
                "You failed to complete a quest for [{}] in time.",
                civ.civ_name
            ),
            vec![NotificationAction::Diplomacy(civ.civ_name.clone())],
            NotificationCategory::Diplomacy,
        ...
        civ.add_notification(
            &format!(
                "[{}] failed to complete your quest in time.",
                assignee.civ_name
            ),
            vec![NotificationAction::Diplomacy(assignee.civ_name.clone())],
            NotificationCategory::Diplomacy,
