...
impl UnitType {
    ...
    pub fn get_movement_type(&self) -> Option<UnitMovementType> {
        self.movement_type
            .as_ref()
            .and_then(|s| UnitMovementType::from_str(s))
    }
    ...
    pub fn is_used(&self, ruleset: &Ruleset) -> bool {
        ruleset
            .units
            .values()
            .any(|unit| unit.unit_type == self.name)
    }
...
impl UnitType {
    ...
    pub fn get_civilopedia_iterator(ruleset: &Ruleset) -> Vec<UnitType> {
        ...
        for movement_type in [
            ...
            result.push(UnitType::new(
                name.to_string(),
                Some(format!("{:?}", movement_type)),
            ));
        }
...
mod tests {
    ...
    fn test_unit_movement_type() {
        assert_eq!(
            UnitMovementType::from_str("Land"),
            Some(UnitMovementType::Land)
        );
        assert_eq!(
            UnitMovementType::from_str("Water"),
            Some(UnitMovementType::Water)
        );
        assert_eq!(
            UnitMovementType::from_str("Air"),
            Some(UnitMovementType::Air)
        );
