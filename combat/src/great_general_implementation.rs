...
pub mod GreatGeneralImplementation {
    ...
    pub fn migrate_great_general_pools(game_info: &mut GameInfo) {
        for civ in &mut game_info.civilizations {
            if civ.great_people.points_for_next_great_general
                >= *civ
                    .great_people
                    .points_for_next_great_general_counter
                    .get("Great General")
                    .unwrap_or(&0)
            {
                civ.great_people
                ...
                civ.great_people.points_for_next_great_general = *civ
                    .great_people
                    .points_for_next_great_general_counter
                    .get("Great General")
                    .unwrap_or(&0);
    ...
    pub fn get_great_general_bonus(
        ...
        let all_generals: Vec<_> = civ_info
            .units
            .get_civ_units()
            .iter()
        ...
        let great_generals: Vec<_> = all_generals
            .iter()
            .flat_map(|general| {
                general
                    .get_matching_uniques(UniqueType::StrengthBonusInRadius, &state)
                    .iter()
            ...
            .filter(|data| {
                ...
                data.general
                    .current_tile
                    .aerial_distance_to(unit.get_tile())
                    <= data.radius
                    && (data.filter == "Military" || unit.matches_filter(&data.filter))
            })
            .collect();

        let great_general_modifier = match great_generals.iter().max_by_key(|data| data.bonus) {
            Some(data) => data,
        ...
        if unit.has_unique(UniqueType::GreatGeneralProvidesDoubleCombatBonus, true)
            && great_general_modifier
                .general
                .is_great_person_of_type("War")
        {
            // apply only on "true" generals
            return (
                great_general_modifier.general.name.clone(),
                great_general_modifier.bonus * 2,
            );
        }

        (
            great_general_modifier.general.name.clone(),
            great_general_modifier.bonus,
        )
    }
    ...
    pub fn get_best_affected_troops_tile(general: &MapUnit) -> Option<Arc<Tile>> {
        ...
        let mut general_bonus_data: Vec<_> = general
            .get_matching_uniques(UniqueType::StrengthBonusInRadius)
            .iter()
            .map(|unique| GeneralBonusData::new(general.clone(), unique))
            .collect();

        // Sort by bonus (descending) and then by radius
        general_bonus_data.sort_by(|a, b| b.bonus.cmp(&a.bonus).then(a.radius.cmp(&b.radius)));
        ...
        let military_unit_tiles_in_distance: Vec<_> = general
            .movement
                ...
                if let Some(military_unit) = &tile.military_unit {
                    military_unit.civ == general.civ
                        && (tile.civilian_unit.is_none()
                            || tile.civilian_unit.as_ref().map_or(false, |u| u == general))
        ...
        let unit_bonus_radius = general_bonus_data.iter().map(|data| data.radius).max()?;
        ...
        military_unit_tiles_in_distance
            ...
            .max_by_key(|unit_tile| {
                unit_tile
                    .get_tiles_in_distance(unit_bonus_radius)
                    .iter()
                    .map(|affected_tile| {
                        if let Some(military_unit) = &affected_tile.military_unit {
                            if military_unit.civ != general.civ || military_unit.is_embarked() {
                                ...
                                let has_attackable_enemies =
                                    military_unit_to_has_attackable_enemies
                                ...
                                if *has_attackable_enemies {
                                    ...
                                    general_bonus_data
                                        ...
                                        .find(|data| {
                                            ...
                                            affected_tile.aerial_distance_to(unit_tile)
                                                <= data.radius
                                                && (data.filter == "Military"
                                                    || military_unit.matches_filter(&data.filter))
                                        })
                                        .map(|data| data.bonus)
                                        .unwrap_or(0)
                                }
