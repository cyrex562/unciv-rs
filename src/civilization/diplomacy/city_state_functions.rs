...
use crate::logic::battle::CityCombatant;
use crate::logic::city::managers::SpyFleeReason;
use crate::models::Spy;
use crate::models::SpyAction;
use crate::models::ruleset::Ruleset;
use crate::models::ruleset::nation::CityStateType;
...
use crate::civilization::{
    AlertType, CivFlags, Civilization, DiplomacyAction, LocationAction, MapUnitAction,
    NotificationCategory, NotificationIcon, PlayerType, PopupAlert, Proximity,
    RelationshipLevel, DiplomaticStatus, DiplomaticModifiers, DeclareWarReason, WarType
};
...
impl<'a> CityStateFunctions<'a> {
    ...
    pub fn init_city_state(&self, ruleset: &Ruleset, starting_era: &str, used_major_civs: &[String]) -> bool {
        let all_mercantile_resources: Vec<String> = ruleset.tile_resources.values()
        ...
        unique_types.extend(city_state_type.friend_bonus_unique_map.get_all_uniques()
            .filter_map(|it| it.type_));
        unique_types.extend(city_state_type.ally_bonus_unique_map.get_all_uniques()
            .filter_map(|it| it.type_));

        // CS Personality
        self.civ_info.city_state_personality = CityStatePersonality::entries().choose(&mut rand::thread_rng())
            .cloned()
        ...
        if unique_types.contains(&UniqueType::CityStateUniqueLuxury) {
            self.civ_info.city_state_resource = all_mercantile_resources.choose(&mut rand::thread_rng())
                .cloned();
        }

        // Unique unit for militaristic city-states
        if unique_types.contains(&UniqueType::CityStateMilitaryUnits) {
            let possible_units: Vec<&BaseUnit> = ruleset.units.values()
                .filter(|it| {
            ...
            if !possible_units.is_empty() {
                self.civ_info.city_state_unique_unit = Some(possible_units.choose(&mut rand::thread_rng())
                    .unwrap()
                    .name
                    .clone());
            }
    ...
    pub fn hold_elections(&self) {
        self.civ_info.add_flag(CivFlags::TurnsTillCityStateElection.name(),
            self.civ_info.game_info.ruleset.mod_options.constants.city_state_election_turns);
        ...
        let spies: Vec<&Spy> = capital.espionage.get_all_stationed_spies()
        ...
        fn get_votes_from_spy(spy: Option<&Spy>, civ_info: &Civilization) -> f32 {
            match spy {
                None => 20.0,
                Some(spy) => {
                    let votes = (civ_info.get_diplomacy_manager_or_meet(&spy.civ_info).influence / 2.0) as f32;
        ...
        let random_seed = (capital.location.x * capital.location.y as f32 + 123.0 * self.civ_info.game_info.turns as f32) as u64;
        let mut rng = rand::rngs::StdRng::seed_from_u64(random_seed);

        let winner = random_weighted(&mut rng, &parties, |spy| get_votes_from_spy(*spy, self.civ_info))
            .and_then(|spy| spy.map(|s| s.civ_info.clone()));

        // There may be no winner, in that case all spies will lose 5 influence
        if let Some(winner) = winner {
            let ally_civ = self.civ_info.get_ally_civ()
            ...
            for civ in self.civ_info.get_known_civs() {
                ...
                self.civ_info.get_diplomacy_manager(civ).unwrap().add_influence(influence);

                if civ == &winner {
                    civ.add_notification(
                        &format!("Your spy successfully rigged the election in [{}]!", self.civ_info.civ_name),
                        capital.location,
                    ...
                    civ.add_notification(
                        &format!("Your spy lost the election in [{}] to [{}]!",
                    ...
                    civ.add_notification(
                        &format!("The election in [{}] were rigged by [{}]!",
                            self.civ_info.civ_name, winner.civ_name),
                        capital.location,
                        NotificationCategory::Espionage,
                        NotificationIcon::Spy
            ...
            for spy in spies {
                self.civ_info.get_diplomacy_manager(&spy.civ_info).unwrap().add_influence(-5.0);
    ...
    pub fn turns_for_great_person_from_city_state(&self) -> i32 {
        ((37 + rand::thread_rng().gen_range(0..7)) as f32 * self.civ_info.game_info.speed.modifier) as i32
    }
    ...
    pub fn give_great_person_to_patron(&self, receiving_civ: &mut Civilization) {
        ...
        let giftable_units: Vec<&BaseUnit> = self.civ_info.game_info.ruleset.units.values()
            .filter(|it| it.is_great_person && !it.has_unique(UniqueType::MayFoundReligion))
            .collect();

        if giftable_units.is_empty() { // For badly defined mods that don't have great people but do have the policy that makes city states grant them
            return;
        }
        ...
        let placed_unit = match receiving_civ.units.place_unit_near_tile(cities.city1.location, gifted_unit) {
            Some(unit) => unit,
            None => return,
        };
        ...
        receiving_civ.add_notification(
            &format!("[{}] gave us a [{}] as a gift!", self.civ_info.civ_name, gifted_unit.name),
            locations,
    ...
    pub fn give_military_unit_to_patron(&self, receiving_civ: &mut Civilization) {
        ...
        fn giftable_unique_unit(civ_info: &Civilization, receiving_civ: &Civilization) -> Option<&BaseUnit> {
            let unique_unit = civ_info.game_info.ruleset.units.get(&civ_info.city_state_unique_unit?)?;
        ...
        fn random_giftable_unit(city: &City, receiving_civ: &Civilization) -> Option<&BaseUnit> {
            city.city_constructions.get_constructable_units()
                .filter(|it| !it.is_civilian() && it.is_land_unit && it.unique_to.is_none())
                .filter(|it| it.get_resource_requirements_per_turn(&receiving_civ.state)
                    .all(|req| req.value <= 0 || receiving_civ.get_resource_amount(&req.key) >= req.value))
                .collect::<Vec<_>>()
        ...
        let placed_unit = match receiving_civ.units.place_unit_near_tile(city.location, &military_unit.name) {
            Some(unit) => unit,
            None => return,
        };

        // The unit should have bonuses from Barracks, Alhambra etc as if it was built in the CS capital
        military_unit.add_construction_bonuses(&placed_unit, &self.civ_info.get_capital().unwrap().city_constructions);
    ...
    pub fn influence_gained_by_gift(&self, donor_civ: &Civilization, gift_amount: i32) -> i32 {
        ...
        influence_gained *= self.civ_info.quest_manager.get_investment_multiplier(&donor_civ.civ_name);
    ...
    pub fn receive_gold_gift(&self, donor_civ: &mut Civilization, gift_amount: i32) {
        ...
        self.civ_info.get_diplomacy_manager(donor_civ).unwrap()
    ...
    pub fn get_protector_civs(&self) -> Vec<&Civilization> {
        ...
        self.civ_info.diplomacy.values()
            .filter(|it| !it.other_civ().is_defeated() && it.diplomatic_status == DiplomaticStatus::Protector)
            .map(|it| it.other_civ())
            .collect()
    }
    ...
    pub fn add_protector_civ(&self, other_civ: &Civilization) {
        ...
        let diplomacy = self.civ_info.get_diplomacy_manager(&other_civ.civ_name).unwrap();
    ...
    pub fn other_civ_can_pledge_protection(&self, other_civ: &Civilization) -> bool {
        ...
        if !self.civ_info.is_city_state || !other_civ.is_major_civ() || other_civ.is_defeated() || !self.civ_info.knows(other_civ) {
            return false;
        }
        let diplomacy = self.civ_info.get_diplomacy_manager(other_civ).unwrap();
    ...
    pub fn update_ally_civ_for_city_state(&self) {
        ...
        let new_ally_name = self.civ_info.diplomacy
            .iter()
        ...
        if let Some(new_ally_name) = new_ally_name {
            ...
            for unique in new_ally_civ.get_matching_uniques(UniqueType::CityStateCanBeBoughtForGold)
                .chain(new_ally_civ.get_matching_uniques(UniqueType::CityStateCanBeBoughtForGoldOld))
            {
                new_ally_civ.get_diplomacy_manager(self.civ_info).unwrap()
                    .set_flag(DiplomacyFlags::MarriageCooldown, unique.params[0].parse::<i32>().unwrap_or(0));
            }

            // Join the wars of our new ally - loop through all civs they are at war with
            for new_enemy in self.civ_info.game_info.civilizations.iter()
                .filter(|it| it.is_at_war_with(&new_ally_civ) && it.is_alive())
            {
                if !self.civ_info.is_at_war_with(new_enemy) {
                    if !self.civ_info.knows(new_enemy) {
                        ...
                        self.civ_info.diplomacy_functions.make_civilizations_meet(new_enemy, true);
                    }
                    self.civ_info.get_diplomacy_manager(new_enemy).unwrap()
                        .declare_war(DeclareWarReason::new(WarType::CityStateAllianceWar, &new_ally_civ));
                }
            }
        }
        ...
        if let Some(old_ally_name) = old_ally_name {
            ...
            if let Some(new_ally_name) = &new_ally_name {
                if old_ally_civ.knows_by_name(new_ally_name) {
                    let diplomacy_manager = old_ally_civ.get_diplomacy_manager_by_name(new_ally_name).unwrap();
    ...
    pub fn get_notification_actions(&self) -> Vec<Box<dyn NotificationAction>> {
        ...
        actions.push(Box::new(DiplomacyAction::new(self.civ_info.civ_name.clone())));
    ...
    pub fn can_be_married_by(&self, other_civ: &Civilization) -> bool {
        !self.civ_info.is_defeated()
            ...
            && self.civ_info.get_diplomacy_manager(other_civ).unwrap()
                .is_relationship_level_eq(RelationshipLevel::Ally)
            && !other_civ.get_diplomacy_manager(self.civ_info).unwrap()
                .has_flag(DiplomacyFlags::MarriageCooldown)
            && (other_civ.get_matching_uniques(UniqueType::CityStateCanBeBoughtForGold).next().is_some()
                || other_civ.get_matching_uniques(UniqueType::CityStateCanBeBoughtForGoldOld).next().is_some())
            && other_civ.gold >= self.get_diplomatic_marriage_cost()
    }

    pub fn diplomatic_marriage(&self, other_civ: &mut Civilization) {
        if !self.can_be_married_by(other_civ) {  // Just in case
            return;
        }
        ...
        other_civ.add_notification(
            &format!("We have married into the ruling family of [{}], bringing them under our control.",
                self.civ_info.civ_name),
        ...
        for city in self.civ_info.game_info.get_cities()
        ...
        for city in &self.civ_info.cities {
            city.espionage.remove_all_present_spies(SpyFleeReason::CityTakenOverByMarriage);
    ...
    pub fn get_tribute_modifiers(&self, demanding_civ: &Civilization, demanding_worker: bool, require_whole_list: bool) -> HashMap<String, i32> {
        ...
        if self.get_protector_civs().iter().any(|it| it != demanding_civ) {
        ...
        if self.civ_info.get_diplomacy_manager(demanding_civ).unwrap().get_influence() < -30.0 {
        ...
        let force_rank = self.civ_info.game_info.get_alive_major_civs()
            .iter()
            .enumerate()
            .find(|(_, civ)| *civ == demanding_civ)
            .map(|(i, _)| i)
            .unwrap_or(0);
        let global_modifier = self.civ_info.game_info.ruleset.mod_options.constants.tribute_global_modifier;
        modifiers.insert(
            "Military Rank".to_string(),
            global_modifier - ((global_modifier / self.civ_info.game_info.game_parameters.players.len() as i32) * force_rank as i32)
        );
        ...
        let in_range_tiles = self.civ_info.get_capital().unwrap()
            .get_center_tile()
            .get_tiles_in_distance_range(1..=bully_range);
        let force_near_city = in_range_tiles
            .iter()
            .map(|it| if let Some(military_unit) = it.military_unit.as_ref() {
        ...
        let local_modifier = self.civ_info.game_info.ruleset.mod_options.constants.tribute_local_modifier;
    ...
    pub fn gold_gained_by_tribute(&self) -> i32 {
        ...
        let turns_to_increment = self.civ_info.game_info.speed.city_state_tribute_scaling_interval;
    ...
    pub fn tribute_gold(&self, demanding_civ: &mut Civilization) {
        ...
        self.civ_info.get_diplomacy_manager(demanding_civ).unwrap().add_influence(-15.0);
    ...
    pub fn tribute_worker(&self, demanding_civ: &mut Civilization) {
        ...
        let buildable_worker_like_units: Vec<&BaseUnit> = self.civ_info.game_info.ruleset.units.values()
            .filter(|it| it.value.has_unique(UniqueType::BuildImprovements)
        ...
        demanding_civ.units.place_unit_near_tile(
            ...
            buildable_worker_like_units.choose(&mut rand::thread_rng()).unwrap()
        );

        self.civ_info.get_diplomacy_manager(demanding_civ).unwrap().add_influence(-50.0);
        self.city_state_bullied(demanding_civ);
        self.civ_info.add_flag(CivFlags::RecentlyBullied.name(), 20);
    }
    ...
    pub fn can_provide_stat(&self, stat_type: Stat) -> bool {
        ...
        for bonus in self.get_city_state_bonuses(&self.civ_info.city_state_type, RelationshipLevel::Ally) {
    ...
    pub fn update_diplomatic_relationship_for_city_state(&self) {
            ...
            .collect::<Vec<_>>()
        {
            ...
            let units_in_border = other_civ.units.get_civ_units()
            ...
            if units_in_border > 0 && diplomacy.is_relationship_level_lt(RelationshipLevel::Friend) {
                ...
                if !diplomacy.has_flag(DiplomacyFlags::BorderConflict) {
                    other_civ.popup_alerts.push(PopupAlert::new(AlertType::BorderConflict, self.civ_info.civ_name.clone()));
    ...
    pub fn get_free_tech_for_city_state(&self) {
        ...
        let researchable_techs: Vec<String> = self.civ_info.game_info.ruleset.technologies.keys()
        ...
        for tech in researchable_techs {
            ...
            if alive_major_civs.iter()
                .filter(|it| it.tech.is_researched(&tech))
                .count() >= alive_major_civs.len() / 2
            {
    ...
    pub fn get_num_threatening_barbarians(&self) -> i32 {
        ...
        let barbarian_civ = self.civ_info.game_info.civilizations.iter()
            .find(|it| it.is_barbarian)
            .unwrap_or_else(|| return 0);
        barbarian_civ.units.get_civ_units()
    ...
    pub fn threatening_barbarian_killed_by(&self, other_civ: &mut Civilization) {
        ...
        other_civ.add_notification(
            &format!("[{}] is grateful that you killed a Barbarian that was threatening them!",
                self.civ_info.civ_name),
            vec![Box::new(DiplomacyAction::new(self.civ_info.civ_name.clone()))],
            NotificationCategory::Diplomacy,
            &[&self.civ_info.civ_name]
        );
    }
    ...
    fn city_state_bullied(&self, bully: &Civilization) {
        ...
        for protector in self.get_protector_civs() {
            ...
            {
                protector_diplomacy.add_modifier(DiplomaticModifiers::BulliedProtectedMinor, -10.0); // Penalty less severe for second offence
            } else {
            ...
            if protector.player_type != PlayerType::Human {   // Humans can have their own emotions
        ...
        self.civ_info.get_diplomacy_manager(bully).unwrap().set_flag(DiplomacyFlags::Bullied, 20);

        // Notify all City-States that we were bullied (for quests)
        for city_state in self.civ_info.game_info.get_alive_city_states() {
            city_state.quest_manager.city_state_bullied(self.civ_info, bully);
        }
    }
    ...
    pub fn city_state_attacked(&self, attacker: &Civilization) {
        ...
        if attacker.is_minor_civ_warmonger() { // They've attacked a lot of city-states
            self.civ_info.get_diplomacy_manager(attacker).unwrap().become_wary();
        }
        else if attacker.is_minor_civ_aggressor() { // They've attacked a few
        ...
        self.civ_info.get_diplomacy_manager(attacker).unwrap()
    ...
    fn make_other_city_states_wary_of_attacker(&self, attacker: &Civilization) {
        for city_state in self.civ_info.game_info.get_alive_city_states() {
            if city_state == self.civ_info { // Must be a different minor
                continue;
            }
            if city_state.get_ally_civ().as_ref().map_or(false, |name| name == &attacker.civ_name) { // Must not be allied to the attacker
                continue;
            }
            if !city_state.knows(attacker) { // Must have met
                continue;
            }
            if city_state.quest_manager.wants_dead(&self.civ_info.civ_name) {  // Must not want us dead
                continue;
            }
            ...
            if rand::thread_rng().gen_range(0..100) <= probability {
                city_state.get_diplomacy_manager(attacker).unwrap().become_wary();
            }
        }
    }
    ...
    fn trigger_protector_civs(&self, attacker: &Civilization) {
        for protector in self.get_protector_civs() {
            ...
            {
                protector_diplomacy.add_modifier(
            ...
            if protector.player_type != PlayerType::Human {   // Humans can have their own emotions
                attacker.add_notification(
                    &format!("[{}] is upset that you attacked [{}], whom they have pledged to protect!",
                        protector.civ_name, self.civ_info.civ_name),
                    NotificationCategory::Diplomacy,
                    NotificationIcon::Diplomacy,
                    &[&protector.civ_name]
    ...
    fn trigger_ally_civs(&self, attacker: &Civilization) {
        if let Some(ally_civ_name) = self.civ_info.get_ally_civ() {
            ...
            if !self.get_protector_civs().contains(&ally_civ) && ally_civ.knows(attacker) {
                ...
                if ally_civ.player_type != PlayerType::Human {   // Humans can have their own emotions
                    attacker.add_notification(
                        &format!("[{}] is upset that you attacked [{}], whom they are allied with!",
    ...
    pub fn city_state_destroyed(&self, attacker: &Civilization) {
        ...
        for protector in self.get_protector_civs() {
            ...
            if protector_diplomacy.has_modifier(DiplomaticModifiers::DestroyedProtectedMinor) {
                protector_diplomacy.add_modifier(DiplomaticModifiers::DestroyedProtectedMinor, -10.0); // Penalty less severe for second offence
            } else {
                protector_diplomacy.add_modifier(DiplomaticModifiers::DestroyedProtectedMinor, -40.0); // Oof
            }
            protector_diplomacy.set_flag(DiplomacyFlags::RememberDestroyedProtectedMinor, 125);   // Reset their memory

            if protector.player_type != PlayerType::Human {   // Humans can have their own emotions
                attacker.add_notification(
                    &format!("[{}] is outraged that you destroyed [{}], whom they had pledged to protect!",
                        protector.civ_name, self.civ_info.civ_name),
                    NotificationCategory::Diplomacy,
                    NotificationIcon::Diplomacy,
                    &[&protector.civ_name]
            ...
            protector.add_notification(
                &format!("[{}] has destroyed [{}], whom you had pledged to protect!",
        ...
        for city_state in self.civ_info.game_info.get_alive_city_states() {
            city_state.quest_manager.city_state_conquered(self.civ_info, attacker);
        }
    }
    ...
    pub fn ask_for_unit_gifts(&self, attacker: &Civilization) {
        if attacker.is_defeated() || self.civ_info.is_defeated() { // never mind, someone died
            return;
        }
        if self.civ_info.cities.is_empty() { // Can't receive units with no cities
            return;
        }

        for third_civ in self.civ_info.get_known_civs()
            .filter(|it| it != attacker && it.is_alive() && it.knows(attacker) && !it.is_at_war_with(attacker) && it.is_major_civ())
        {
        ...
        self.civ_info.get_known_civs()
            ...
            .flat_map(|city_state| {
                ...
                let relationship_level =
                    if city_state.get_ally_civ().as_ref().map_or(false, |name| name == &self.civ_info.civ_name) {
                        RelationshipLevel::Ally
                    } else if city_state.get_diplomacy_manager(self.civ_info).unwrap().get_influence() >= 30.0 {
                        RelationshipLevel::Friend
                    } else {
                        RelationshipLevel::Neutral
                    };
                self.get_city_state_bonuses(&city_state.city_state_type, relationship_level, Some(unique_type))
            })
