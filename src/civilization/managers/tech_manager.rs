...
use crate::civilization::{Civilization, AlertType, LocationAction, NotificationCategory, NotificationIcon, PlayerType, PopupAlert, TechAction};
use crate::city::City;
use crate::constants::Constants;
use crate::map::tile::RoadStatus;
use crate::models::ruleset::{
    INonPerpetualConstruction, Era, Technology, TileResource,
...
impl TechManager {
    ...
    fn get_science_modifier(&self, tech_name: &str) -> f32 {
        ...
        let number_of_civs_researched_this_tech = civ.get_known_civs()
            .iter()
            .filter(|it| it.is_major_civ() && it.tech.is_researched(tech_name))
            .count();
        let number_of_civs_remaining = civ.game_info.civilizations
            .iter()
    ...
    pub fn cost_of_tech(&self, tech_name: &str) -> i32 {
        ...
        let map_size_predef = civ.game_info.tile_map.map_parameters.map_size.get_predefined_or_next_smaller();
        tech_cost *= map_size_predef.tech_cost_multiplier;

        let mut city_modifier = (civ.cities.iter().filter(|it| !it.is_puppet()).count() - 1) as f32 * map_size_predef.tech_cost_per_city_modifier;
    ...
    pub fn current_technology(&self) -> Option<&Technology> {
        ...
        Some(self.get_ruleset().technologies.get(current_technology_name)?)
    }
    ...
    pub fn turns_to_tech(&self, tech_name: &str) -> String {
        ...
        if remaining_cost <= 0.0 {
            ...
            let turns = (remaining_cost / civ.stats.stats_for_next_turn.science).ceil().max(1.0) as i32;
            turns.to_string()
        }
    }
    ...
    pub fn is_researched_construction(&self, construction: &dyn INonPerpetualConstruction) -> bool {
        construction.required_techs().iter().all(|required_tech| self.is_researched(required_tech))
    }
    ...
    pub fn is_obsolete(&self, unit: &BaseUnit) -> bool {
        unit.techs_that_obsolete_this().iter().any(|obsolete_tech| self.is_researched(obsolete_tech))
    }

    pub fn is_unresearchable(&self, tech: &Technology) -> bool {
        let civ = self.civ.as_ref().expect("Civ not set");

        if tech.get_matching_uniques(UniqueType::OnlyAvailable, &StateForConditionals::ignore_conditionals())
            .iter()
    ...
    pub fn can_be_researched(&self, tech_name: &str) -> bool {
        let tech = self.get_ruleset().technologies.get(tech_name).expect("Tech not found");
        ...
        tech.prerequisites.iter().all(|prereq| self.is_researched(prereq))
    }
    ...
    pub fn get_required_techs_to_destination(&self, destination_tech: &Technology) -> Vec<&Technology> {
        ...
        while !check_prerequisites.is_empty() {
            ...
            if !tech_to_check.is_continually_researchable() &&
                (self.is_researched(&tech_to_check.name) || prerequisites.contains(&tech_to_check)) {
                continue; //no need to add or check prerequisites
            }

            for prerequisite in &tech_to_check.prerequisites {
                check_prerequisites.push_back(self.get_ruleset().technologies.get(prerequisite).unwrap());
            }
    ...
    pub fn get_science_from_great_scientist(&self) -> i32 {
        ...
        let civ = self.civ.as_ref().expect("Civ not set");
        (self.science_of_last_8_turns.iter().sum::<i32>() as f32 * civ.game_info.speed.science_cost_modifier) as i32
    }
    ...
    fn limit_overflow_science(&self, overflow_science: i32) -> i32 {
        ...
        let civ = self.civ.as_ref().expect("Civ not set");
        let current_tech_name = self.current_technology_name().expect("No current technology");
        let current_tech = self.get_ruleset().technologies.get(current_tech_name).expect("Tech not found");

        let max_overflow = (civ.stats.stats_for_next_turn.science as i32 * 5).max(current_tech.cost);
    ...
    pub fn end_turn(&mut self, science_for_new_turn: i32) {
        ...
        if self.science_from_research_agreements != 0 {
            let science_boost = this.science_from_research_agreements();
            final_science_to_add += science_boost;
            self.science_from_research_agreements = 0;

            let civ = self.civ.as_ref().expect("Civ not set");
            civ.add_notification(
                format!("We gained [{}] Science from Research Agreement", science_boost),
        ...
        this.add_science(final_science_to_add);
    }

    pub fn add_science(&mut self, science_get: i32) {
        let current_technology = match this.current_technology_name() {
            Some(name) => name,
            None => return,
        };

        let current_progress = this.research_of_tech(Some(current_technology));
        this.techs_in_progress.insert(current_technology.to_string(), current_progress + science_get);

        if this.techs_in_progress.get(current_technology).unwrap() < &this.cost_of_tech(current_technology) {
            return;
        }

        // We finished it!
        // http://www.civclub.net/bbs/forum.php?mod=viewthread&tid=123976
        let extra_science_left_over = this.techs_in_progress.get(current_technology).unwrap() - this.cost_of_tech(current_technology);
        this.overflow_science += this.limit_overflow_science(extra_science_left_over);
        this.add_technology(current_technology);
    }
    ...
    pub fn update_research_progress(&mut self) {
        ...
        let real_overflow = this.get_overflow_science();
        let science_spent = this.research_of_tech(Some(current_technology)) + real_overflow;

        if science_spent >= this.cost_of_tech(current_technology) {
            this.overflow_science = 0;
            if real_overflow != 0 {
                this.add_science(real_overflow);
            }
        }
    }
    ...
    pub fn get_free_technology(&mut self, tech_name: &str) {
        this.free_techs -= 1;
        this.add_technology(tech_name);
    }

    pub fn add_technology(&mut self, tech_name: &str, show_notification: bool) {
        let civ = self.civ.as_ref().expect("Civ not set");
        let is_new_tech = this.techs_researched.insert(tech_name.to_string());

        // this is to avoid concurrent modification problems
        let new_tech = this.get_ruleset().technologies.get(tech_name).expect("Tech not found");

        if !new_tech.is_continually_researchable() {
            this.techs_to_research.retain(|t| t != tech_name);
        } else {
            this.repeating_techs_researched += 1;
        }

        this.techs_in_progress.remove(tech_name);
        this.researched_technologies.push(new_tech.clone());
        this.add_tech_to_transients(new_tech);

        this.move_to_new_era(show_notification);
        ...
        if is_new_tech {
            civ.popup_alerts.push(PopupAlert::new(AlertType::TechResearched, tech_name));
        }
        ...
        for unique in civ.get_triggered_uniques(UniqueType::TriggerUponResearch)
            .iter()
        ...
        let revealed_resources: Vec<&TileResource> = this.get_ruleset().tile_resources.values()
        ...
        if civ.player_type == PlayerType::Human {
            for revealed_resource in revealed_resources {
                civ.game_info.notify_explored_resources(civ.clone(), revealed_resource.name.clone(), 5);
            }
        }

        this.update_transient_booleans();
        ...
        this.obsolete_old_units(tech_name);
        ...
        this.update_research_progress();
    }

    /** A variant of kotlin's [associateBy] that omits null values */
    fn associate_by_not_null<T, K, V, F, G>(iter: impl Iterator<Item = T>, key_selector: F, value_transform: G) -> HashMap<K, V>
    where
        F: Fn(&T) -> K,
        G: Fn(&T) -> Option<V>,
        K: std::hash::Hash + Eq,
    ...
    fn obsolete_old_units(&mut self, tech_name: &str) {
        ...
        fn get_equivalent_upgrade_or_null(unit: &BaseUnit, tech_name: &str, civ: &Civilization) -> Option<BaseUnit> {
            let unit_upgrades_to = unit.automatically_upgraded_in_production_to_unit_by_tech(tech_name)?;
            Some(civ.get_equivalent_unit(unit_upgrades_to))
        }

        let obsolete_units = Self::associate_by_not_null(
            this.get_ruleset().units.entries.iter(),
        ...
        fn transform_construction(old: &str, city: &City, unit_upgrades: &mut HashMap<String, HashSet<&City>>, obsolete_units: &HashMap<String, BaseUnit>) -> Option<String> {
        ...
        for city in &civ.cities {
            ...
            for construction in &city.city_constructions.construction_queue {
                if let Some(new_construction) = transform_construction(construction, city, &mut unit_upgrades, &obsolete_units) {
        ...
        for (unit, cities) in unit_upgrades {
            ...
            let location_action = LocationAction::new(cities.iter().map(|city| city.location).collect());
            ...
            let text = if new_unit.is_none() {
                format!("[{}] has become obsolete and was removed from the queue in {}!", unit, city_text)
            } else {
                format!("{} changed production from [{}] to [{}]", city_text, unit, new_unit.unwrap())
            };

            let icons = if new_unit.is_none() {
                vec![NotificationIcon::Construction]
            } else {
                vec![NotificationIcon::from_str(&unit).unwrap(), NotificationIcon::Construction, NotificationIcon::from_str(&new_unit.unwrap()).unwrap()]
            };

            civ.add_notification(text, location_action, NotificationCategory::Production, icons);
        }
    }
    ...
    fn move_to_new_era(&mut self, show_notification: bool) {
        ...
        if show_notification {
            ...
            for policy_branch in this.get_ruleset().policy_branches.values()
                .filter(|branch| branch.era == current_era.name && civ.policies.is_adoptable(branch)) {
                if !civ.is_spectator() {
                    civ.add_notification(
        ...
        let eras_passed: Vec<&Era> = this.get_ruleset().eras.values()
            .filter(|era| era.era_number > previous_era.era_number && era.era_number <= current_era.era_number)
            .collect();
        ...
        for unique in civ.get_triggered_uniques(UniqueType::TriggerUponEnteringEra) {
            for era_name in &era_names {
                if unique.get_modifiers(UniqueType::TriggerUponEnteringEra)
                    .iter()
    ...
    fn update_era(&mut self) {
        let civ = self.civ.as_ref().expect("Civ not set");
        let ruleset = this.get_ruleset();

        if ruleset.technologies.is_empty() || this.researched_technologies.is_empty() {
            return;
        }

        let max_era_of_researched_techs = this.researched_technologies
            .iter()
        ...
        let min_era_of_non_researched_techs = ruleset.technologies.values
            .iter()
            .filter(|tech| !this.researched_technologies.contains(tech))
            .map(|tech| tech.column.as_ref().unwrap())
            .min_by_key(|col| col.column_number)
            .map(|col| col.era);

        if min_era_of_non_researched_techs.is_none() {
            this.era = max_era.clone();
            return;
        }

        let min_era = ruleset.eras.get(&min_era_of_non_researched_techs.unwrap()).unwrap();

        this.era = if min_era.era_number <= max_era.era_number {
            max_era.clone()
        } else {
            min_era.clone()
        };
    ...
    fn add_tech_to_transients(&mut self, tech: &Technology) {
        this.tech_uniques.add_uniques(&tech.unique_objects);
    }

    pub fn set_transients(&mut self, civ: Arc<Civilization>) {
        this.civ = Some(civ.clone());
        this.researched_technologies = this.techs_researched.iter()
            .map(|tech_name| this.get_ruleset().technologies.get(tech_name).unwrap().clone())
            .collect();

        for tech in &this.researched_technologies {
            this.add_tech_to_transients(tech);
        }

        this.update_era();  // before updateTransientBooleans so era-based conditionals can work
        this.update_transient_booleans();
    }
    ...
    fn update_transient_booleans(&mut self) {
        ...
        this.units_can_embark = civ.has_unique(UniqueType::LandUnitEmbarkation);

        let enter_ocean_uniques = civ.get_matching_uniques(UniqueType::UnitsMayEnterOcean);
        this.all_units_can_enter_ocean = enter_ocean_uniques.iter().any(|unique| unique.params[0] == Constants::ALL);
        this.embarked_units_can_enter_ocean = this.all_units_can_enter_ocean ||
            enter_ocean_uniques.iter().any(|unique| unique.params[0] == Constants::EMBARKED);
        this.specific_units_can_enter_ocean = enter_ocean_uniques.iter().any(|unique|
            unique.params[0] != Constants::ALL && unique.params[0] != Constants::EMBARKED);

        this.movement_speed_on_roads = if civ.has_unique(UniqueType::RoadMovementSpeed) {
            RoadStatus::Road.movement_improved
        } else {
            RoadStatus::Road.movement
        };

        this.roads_connect_across_rivers = civ.has_unique(UniqueType::RoadsConnectAcrossRivers);

        this.all_techs_are_researched = civ.game_info.ruleset.technologies.values
            .iter()
            .all(|tech| this.is_researched(&tech.name) || !this.can_be_researched(&tech.name));
    }

    pub fn get_best_road_available(&self) -> RoadStatus {
        let railroad_improvement = this.get_ruleset().railroad_improvement.clone();  // May not exist in mods
        if let Some(railroad) = railroad_improvement {
            if railroad.tech_required.is_none() || this.is_researched(railroad.tech_required.unwrap()) {
                return RoadStatus::Railroad;
            }
        }

        let road_improvement = this.get_ruleset().road_improvement.clone();
        if let Some(road) = road_improvement {
            if road.tech_required.is_none() || this.is_researched(road.tech_required.unwrap()) {
    ...
    pub fn can_research_tech(&self) -> bool {
        this.get_ruleset().technologies.values
            .iter()
            .any(|tech| this.can_be_researched(&tech.name))
    }
}