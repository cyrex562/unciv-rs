...
impl CityExpansionManager {
    ...
    pub fn tiles_claimed(&self) -> i32 {
        if let Some(city) = &this.city {
            let tiles_around_city: Vec<_> = city.get_center_tile().neighbors.iter()
    ...
    pub fn get_culture_to_next_tile(&self) -> i32 {
        if let Some(city) = &this.city {
            let mut culture_to_next_tile = 6.0 * (0.0f32.max(self.tiles_claimed() as f32) + 1.4813).powf(1.3);
    ...
    pub fn can_buy_tile(&self, tile: &Tile) -> bool {
        if let Some(city) = &this.city {
            ...
            tile.neighbors.iter().any(|neighbor| neighbor.get_city().map_or(false, |c| c == city))
        } else {
            false
        }
    }
    ...
    pub fn buy_tile(&mut self, tile: &Tile) -> Result<(), String> {
        if let Some(city) = &this.city {
            ...
            if !tile.neighbors.iter().any(|neighbor| neighbor.get_city().map_or(false, |c| c == city)) {
                return Err(format!("{} tried to buy {}, but it owns none of the neighbors", city.name, tile.position));
            }

            // Check if the city has enough gold
            if city.civ.gold < gold_cost && !city.civ.game_info.game_parameters.god_mode {
                return Err(format!("{} tried to buy {}, but lacks gold (cost {}, has {})",
    ...
    pub fn get_gold_cost_of_tile(&self, tile: &Tile) -> i32 {
        if let Some(city) = &this.city {
            ...
            let mut cost = base_cost as f32 * (distance_from_center - 1) as f32 + self.tiles_claimed() as f32 * 5.0;
    ...
    pub fn get_choosable_tiles(&self) -> Vec<Arc<Tile>> {
        if let Some(city) = &this.city {
            city.get_center_tile().get_tiles_in_distance(city.get_expand_range())
                .into_iter()
    ...
    pub fn choose_new_tile_to_own(&self) -> Option<Arc<Tile>> {
        if let Some(city) = &this.city {
            ...
            self.get_choosable_tiles().into_iter()
                .min_by_key(|tile| Automation::rank_tile_for_expansion(tile, city, &local_unique_cache))
        } else {
            None
        }
    }
    ...
    pub fn reset(&mut self) {
        if let Some(city) = &this.city {
            ...
            for tile in city.get_center_tile().get_tiles_in_distance(1)
                    .into_iter()
                    .filter(|tile| tile.get_city().is_none()) { // can't take ownership of owned tiles (by other cities)
                self.take_ownership(&tile);
            }
        }
    }
    ...
    pub fn take_ownership(&mut self, tile: &Tile) {
        if let Some(city) = &this.city {
            assert!(!tile.is_city_center(), "Trying to found a city in a tile that already has one");
            ...
            for unit in tile.get_units().iter().cloned().collect::<Vec<_>>() { // cloned because we're modifying
                if !unit.civ.diplomacy_functions.can_pass_through_tiles(&city.civ) {
                    ...
                    if unit.cache.has_unique_to_build_improvements || unit.cache.has_unique_to_create_water_improvements {
                        unit.due = true;
                        unit.action = None;
                    }
                }
