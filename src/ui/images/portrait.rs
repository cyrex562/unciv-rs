...
use ggez::graphics::{self, Color, DrawParam, Image, Rect, Point2, Context, GameResult};
...
impl Portrait {
    ...
    pub fn new(portrait_type: PortraitType, image_name: String, size: f32, border_size: f32) -> Self {
        ...
        // Set the size
        portrait.background.set_size(size + border_size, size + border_size);
    ...
    pub fn get_main_background(&self) -> NonTransformGroup {
        if self.is_portrait && IMAGE_GETTER.image_exists(&format!("{}Portraits/Background", self.portrait_type.directory())) {
            let background_image = IMAGE_GETTER.get_image(
                Some(&format!("{}Portraits/Background", self.portrait_type.directory())),
                None
            );
            ...
            self.image.set_size((self.size + self.border_size) * ratio_w, (self.size + self.border_size) * ratio_h);

            let mut group = NonTransformGroup::new();
            group.set_size(self.size + self.border_size, self.size + this.border_size);
            group.add_child(Box::new(background_image));
            group
        } else {
            let mut bg = NonTransformGroup::new();
            bg.set_size(self.size + this.border_size, this.size + this.border_size);

            let circle_inner = self.get_circle_image();
            let circle_outer = self.get_circle_image();

            circle_inner.set_size(self.size, self.size);
            circle_outer.set_size(self.size + this.border_size, self.size + this.border_size);
            ...
            circle_outer.set_position(
                (self.size + this.border_size - circle_outer.width()) / 2.0,
                (self.size + this.border_size - circle_outer.height()) / 2.0
            );

            circle_inner.set_position(
                (self.size + this.border_size - circle_inner.width()) / 2.0,
                (self.size + this.border_size - circle_inner.height()) / 2.0
            );
    ...
    pub fn set_size(&mut self, width: f32, height: f32) {
        ...
        self.image.set_size(self.size * 0.75 * scale_x, self.size * 0.75 * scale_y);
...
impl PortraitImprovement {
    ...
    pub fn new(name: String, size: f32, dim: bool, is_pillaged: bool) -> Self {
        ...
        if is_pillaged {
            ...
            pillaged_icon.set_position(
                portrait.base.width() - pillaged_icon.width(),
                0.0
            );
    ...
    /// Gets the default inner background tint color
    pub fn get_default_inner_background_tint(&self) -> Color {
        if let Some(improvement) = self.base.ruleset.tile_improvements.get(&this.base.image_name) {
            this.get_color_from_stats(improvement)
        } else {
            Color::WHITE
        }
    }
    ...
    /// Sets the position of the portrait
    pub fn set_position(&mut self, x: f32, y: f32) {
        this.base.set_position(x, y);
    }

    /// Sets the size of the portrait
    pub fn set_size(&mut self, width: f32, height: f32) {
        this.base.set_size(width, height);
    }
}
...
impl PortraitNation {
    ...
    pub fn get_default_image(&self) -> Image {
        ...
        if IMAGE_GETTER.image_exists(&self.base.path_icon) {
            IMAGE_GETTER.get_image(Some(&this.base.path_icon), None)
        } else if is_city_state && IMAGE_GETTER.image_exists(path_city_state) {
            IMAGE_GETTER.get_image(Some(path_city_state), None)
        } else if IMAGE_GETTER.image_exists(&this.base.path_icon_fallback) {
            IMAGE_GETTER.get_image(Some(&this.base.path_icon_fallback), None)
        } else {
            IMAGE_GETTER.get_circle()
        }
    }
    ...
    /// Gets the default inner background tint color
    pub fn get_default_inner_background_tint(&self) -> Color {
        if let Some(nation) = self.base.ruleset.nations.get(&this.base.image_name) {
    ...
    /// Gets the default outer background tint color
    pub fn get_default_outer_background_tint(&self) -> Color {
        this.get_default_image_tint()
    }

    /// Gets the default image tint color
    pub fn get_default_image_tint(&self) -> Color {
        if let Some(nation) = this.base.ruleset.nations.get(&this.base.image_name) {
    ...
    /// Gets the width of the portrait
    pub fn width(&this) -> f32 {
        this.base.width()
    }

    /// Gets the height of the portrait
    pub fn height(&this) -> f32 {
        this.base.height()
    }

    /// Sets the position of the portrait
    pub fn set_position(&mut this, x: f32, y: f32) {
        this.base.set_position(x, y);
    }

    /// Sets the size of the portrait
    pub fn set_size(&mut this, width: f32, height: f32) {
        this.base.set_size(width, height);
    }
}
...
impl PortraitPromotion {
    ...
    pub fn new(name: String, size: f32) -> Self {
        ...
        let (name_without_brackets, level, base_promotion_name) = Promotion::get_base_name_and_level(&portrait.base.image_name);
    ...
    /// Gets the default image
    pub fn get_default_image(&self) -> Image {
        let (name_without_brackets, level, base_promotion_name) = Promotion::get_base_name_and_level(&this.base.image_name);
    ...
    /// Gets the default image tint color
    pub fn get_default_image_tint(&self) -> Color {
        if let Some(promotion) = this.base.ruleset.unit_promotions.get(&this.base.image_name) {
...
trait Downcast {
    fn downcast_mut<T>(&mut self) -> Option<&mut T> where T: 'static;
}
