...
impl TileGroup {
    pub fn new(tile: Arc<Tile>, tile_set_strings: TileSetStrings, group_size: Option<f32>) -> Self {
        ...
        let hexagon_image_origin = (
            ...
            (hexagon_image_width / 2.0).powf(2.0) - (hexagon_image_width / 4.0).powf(2.0)
        ).0.sqrt()
        );
    ...
    fn set_all_layers_visible(&mut self, is_visible: bool) {
        ...
        this.layer_unit_flag.set_visible(is_visible);
        this.layer_city_button.set_visible(is_visible);
    }
    ...
    pub fn update(&mut self, viewing_civ: Option<&Civilization>, local_unique_cache: Option<&LocalUniqueCache>) {
        ...
        // Reset overlays
        this.layer_misc.remove_hex_outline();
        this.layer_misc.hide_terrain_overlay();
        this.layer_overlay.hide_highlight();
        this.layer_overlay.hide_crosshair();
        this.layer_overlay.hide_good_city_location_indicator();

        let was_previously_visible = this.layer_terrain.is_visible();

        // Show all layers by default
        this.set_all_layers_visible(true);

        // Do not update layers if tile is not explored by viewing player
        if let Some(viewing_civ) = viewing_civ {
            if !(this.is_force_visible || viewing_civ.has_explored(&this.tile)) {
                this.reset(local_unique_cache);

                // If tile has explored neighbors - reveal layers partially
                if this.tile.neighbors.iter().all(|neighbor| !viewing_civ.has_explored(neighbor)) {
                    // Else - hide all layers
                    this.set_all_layers_visible(false);
                } else {
                    this.layer_overlay.set_unexplored(viewing_civ);
                }
                return;
            }
        }

        this.remove_missing_mod_references();

        // Update all layers
        this.layer_terrain.update(viewing_civ, local_unique_cache);
        this.layer_features.update(viewing_civ, local_unique_cache);
        this.layer_borders.update(viewing_civ, local_unique_cache);
        this.layer_misc.update(viewing_civ, local_unique_cache);
        this.layer_resource.update(viewing_civ, local_unique_cache);
        this.layer_improvement.update(viewing_civ, local_unique_cache);
        this.layer_yield.update(viewing_civ, local_unique_cache);
        this.layer_overlay.update(viewing_civ, local_unique_cache);
        this.layer_unit_art.update(viewing_civ, local_unique_cache);
        this.layer_unit_flag.update(viewing_civ, local_unique_cache);
        this.layer_city_button.update(viewing_civ, local_unique_cache);
    ...
    fn remove_missing_mod_references(&mut self) {
        ...
        let units = this.tile.get_units();
        for unit in units {
            if !this.tile.ruleset.nations.contains_key(&unit.owner) {
    ...
    pub fn draw(&self, ctx: &mut Context) {
        ...
        this.layer_borders.draw(ctx);
        this.layer_misc.draw(ctx);
        this.layer_resource.draw(ctx);
        this.layer_improvement.draw(ctx);
        this.layer_yield.draw(ctx);
        this.layer_overlay.draw(ctx);
