use crate::models::{
    Building, BaseUnit, Civilization, City, CityFocus, CityStats,
    INonPerpetualConstruction, PerpetualConstruction, PersonalityValue,
    ResourceType, TileImprovement, UniqueType, Victory, Stat, Stats
};
use crate::map::{BFS, TileMap, MapUnit, Tile};
use crate::ui::RankingType;
...
impl Automation {
    ...
    pub fn rank_stats_for_city_work(
        ...
        if are_we_ranking_specialist {
            ...
            for unique in local_unique_cache.for_city_get_matching_uniques(city, UniqueType::UnhappinessFromPopulationTypePercentageChange) {
                if unique.params[1] == "Specialists" && city.matches_filter(&unique.params[2]) {
                    yield_stats.happiness -= unique.params[0].parse::<f32>().unwrap() / 100.0;
                }
            }
        }

        let surplus_food = city.city_stats.current_city_stats[Stat::Food];
        ...
        if starving {
            feed_food = (yield_stats.food).min(-surplus_food).max(0.0);
        }
        ...
        // Handle growth food
        if let None = city.get_matching_uniques(UniqueType::NullifiesGrowth).first() {
    ...
    fn provides_unneeded_carrying_slots(base_unit: &BaseUnit, civ_info: &Civilization) -> bool {
        ...
        fn get_carry_amount(map_unit: &MapUnit) -> i32 {
    ...
    pub fn choose_military_unit(city: &City, available_units: impl Iterator<Item = &BaseUnit>) -> Option<String> {
        ...
        fn is_naval_melee_unit(unit: &BaseUnit) -> bool {
            unit.is_melee() && unit.type.is_water_unit()
        }
        ...
        let military_units: Vec<&BaseUnit> = available_units
            ...
            .filter(|it| Self::allow_spending_resource(&city.civ, *it))
            .filter(|it| {
    ...
    pub fn allow_spending_resource(
        ...
        let required_resources = if let Some(unit) = construction.downcast_ref::<BaseUnit>() {
            ...
            construction.get_resource_requirements_per_turn(city_info.map_or(&civ_info.state, |c| &c.state))
        };
    ...
    pub fn get_tile_for_construction_improvement(city: &City, improvement: &TileImprovement) -> Option<&Tile> {
            ...
            .filter(|it| {
                !it.get_tile_improvement()
                    .map_or(false, |imp| imp.has_unique(
                        ...
                        StateForConditionals::new(&city.civ, city, Some(*it))
                    ))
                && it.improvement_functions.can_build_improvement(improvement, &city.civ)
            })
            .max_by_key(|it| Self::rank_tile_for_city_work(it, city, &local_unique_cache))
    }
