...
impl CityTileGroup {
    ...
    pub fn update(&mut self, viewing_civ: Option<&Civilization>, local_unique_cache: &LocalUniqueCache) {
        ...
        } else if self.tile.stats.get_tile_stats(&self.city, &self.city.civ).is_empty() {
        ...
        if let Some(mut icon) = icon {
            ...
            self.layer_misc.add_worked_icon(icon);
        }

        // No unit flags and city-buttons inside CityScreen
        self.layer_unit_flag.set_visible(false);
        self.layer_city_button.set_visible(false);

        // Pixel art, roads, improvements are dimmed inside CityScreen
        self.layer_unit_art.dim();
        self.layer_features.dim();
        self.layer_improvement.dim_improvement(true);

        // Put whole layer (yield, pop, improvement, res) to front
        self.layer_misc.to_front();
    }
...
impl LayerMisc {
    ...
    pub fn remove_worked_icon(&mut self) {
        self.worked_icon = None;
    }

    pub fn add_worked_icon(&mut self, icon: Image) {
        self.worked_icon = Some(icon);
    }
    ...
    pub fn draw(&self, ctx: &mut Context) {
        if let Some(icon) = &self.worked_icon {
...
impl LayerImprovement {
    ...
    pub fn dim_improvement(&mut self, dimmed: bool) {
        self.dim_factor = if dimmed { 0.5 } else { 1.0 };
    }
