...
impl UncivSlider {
    ...
    pub fn new(
        ...
        if plus_minus {
            ...
            unciv_slider
                .base
                .add(minus_button.clone())
                .with_padding(if vertical {
                    (0.0, Self::PADDING, 0.0, 0.0)
                } else {
                    (0.0, 0.0, 0.0, Self::PADDING)
                });
        ...
        if plus_minus {
            ...
            unciv_slider
                .base
                .add(plus_button.clone())
                .with_padding(if vertical {
                    (Self::PADDING, 0.0, 0.0, 0.0)
                } else {
                    (0.0, 0.0, Self::PADDING, 0.0)
                });
        ...
        unciv_slider
            .slider
            .add_change_listener(Box::new(move |event, actor| {
        ...
        match self.tip_type {
            ...
            _ => {
                if let Some(get_tip_text) = &self.get_tip_text {
                    ...
                    self.tip_label
                        .set_text(format!(self.tip_format, self.value()));
                }
                ...
                match self.tip_type {
                    ...
                    TipType::Auto => {
                        ...
                        timer.schedule(
                            Duration::from_secs_f32(Self::HIDE_DELAY),
                            Box::new(move || {
                                self.hide_tip();
                            }),
                        );
    ...
    fn set_plus_minus_enabled(&mut self) {
        ...
        let enable_plus = self.value() < self.max_value() && !self.is_disabled();
        if let Some(plus_button) = &mut self.plus_button {
    ...
    fn step_changed(&mut self) {
        self.tip_format = if self.step_size() > 0.99 {
            "%.0f".to_string()
        } else if self.step_size() > 0.099 {
            "%.1f".to_string()
        } else if self.step_size() > 0.0099 {
            "%.2f".to_string()
        } else {
            "%.3f".to_string()
        };

        if self.tip_type != TipType::None && self.get_tip_text.is_none() {
            self.tip_label
                .set_text(format!(self.tip_format, self.value()));
        }
    }
        ...
        while let Some(parent) = widget.parent() {
            ...
            if !scroll_pane.listeners().is_empty() {
                self.killed_listeners
                    .insert(scroll_pane.clone(), scroll_pane.listeners().to_vec());
            }

            if !scroll_pane.capture_listeners().is_empty() {
                self.killed_capture_listeners.insert(
                    scroll_pane.clone(),
                    scroll_pane.capture_listeners().to_vec(),
                );
            }
    ...
    fn resurrect_scroll_panes(&mut self) {
        let mut widget: &dyn WidgetGroup = this;
        while let Some(parent) = widget.parent() {
            widget = parent;
            if !widget.is_scroll_pane() {
            ...
            if let Some(listeners) = self.killed_listeners.remove(&scroll_pane) {
                for listener in listeners {
                    scroll_pane.add_listener(listener);
                }
            }

            if let Some(listeners) = self.killed_capture_listeners.remove(&scroll_pane) {
                for listener in listeners {
    ...
    fn show_tip(&mut self) {
        if self.tip_container.has_parent() {
            return;
        }

        self.tip_container.pack();
        if self.needs_layout() {
            self.pack();
        }

        let pos = self.slider.local_to_parent_coordinates(Point2 {
            x: self.slider.width() / 2.0,
            y: self.slider.height(),
        });

        self.tip_container.set_origin(ggez::graphics::Align::Bottom);
        self.tip_container
            .set_position(pos.x, pos.y, ggez::graphics::Align::Bottom);
        self.tip_container.set_transform(true);
        self.tip_container.color_mut().a = 0.2;
        self.tip_container.set_scale(0.05);

        self.add_actor(self.tip_container.clone());

        self.tip_container
            .add_action(ggez::graphics::Action::parallel(
                ggez::graphics::Action::fade_in(
                    Self::TIP_ANIMATION_DURATION,
                    ggez::graphics::Interpolation::Fade,
                ),
                ggez::graphics::Action::scale_to(
                    1.0,
                    1.0,
                    0.2,
                    ggez::graphics::Interpolation::Fade,
    ...
    fn hide_tip(&mut self) {
        self.tip_container
            .add_action(ggez::graphics::Action::sequence(
...
impl Clone for UncivSlider {
    fn clone(&self) -> Self {
        Self {
            ...
            minus_button: self.minus_button.clone(),
            plus_button: self.plus_button.clone(),
            tip_label: self.tip_label.clone(),
            tip_container: self.tip_container.clone(),
            tip_hide_task: None, // Don't clone the timer
            snap_to_values: self.snap_to_values.clone(),
            snap_threshold: self.snap_threshold,
            tip_format: self.tip_format.clone(),
            has_focus: self.has_focus,
            block_listener: self.block_listener,
            tip_type: self.tip_type,
            get_tip_text: self.get_tip_text.clone(),
