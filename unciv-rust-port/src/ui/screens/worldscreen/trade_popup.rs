...
impl TradePopup {
    ...
    pub fn show(&self, ui: &mut Ui) {
        ...
        if ui.button("How about something else...").clicked() {
            self.close();
            self.world_screen.borrow().game.push_screen(DiplomacyScreen::new(
    ...
    fn close(&self) {
        self.viewing_civ.borrow_mut().trade_requests.remove(&self.trade_request);
    }
}
