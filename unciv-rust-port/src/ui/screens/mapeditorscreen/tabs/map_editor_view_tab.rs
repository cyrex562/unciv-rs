...
impl MapEditorViewTab {
    pub fn render(&mut self, ui: &mut Ui) {
        ...
        ui.vertical(|ui| {
            ...
            // Natural wonders
            if !self.natural_wonders.is_empty() {
                let mut lines = Vec::new();

                for (wonder, count) in self.natural_wonders.iter() {
                    let text = if *count == 1 {
                        wonder.clone()
                    } else {
                        format!("{{{}}} ({})", wonder, count)
                ...
                ExpanderTab::new(&format!("{{Natural Wonders}} ({})", self.natural_wonders.len()), 21, false, 5.0, |ui| {
