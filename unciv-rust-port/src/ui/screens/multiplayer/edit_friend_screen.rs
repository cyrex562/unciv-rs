...
use crate::multiplayer::friend_list::{ErrorType, Friend, FriendList};
...
impl PickerScreen for EditFriendScreen {
    fn init(&mut self, ui: &mut Ui) {
        ...
        ui.add_space(10.0);
        self.friend_name_text_field.show(ui);
        ui.add_space(30.0);

        // Add player ID field with paste button
        ui.heading("Player ID");
        ui.horizontal(|ui| {
            self.player_id_text_field.show(ui);
            if ui.button("Paste player ID from clipboard").clicked() {
                if let Ok(clipboard_text) = crate::ui::screens::multiplayer::multiplayer_helpers::MultiplayerHelpers::copy_to_clipboard("") {
                    self.player_id_text_field.set_text(&clipboard_text);
                }
            }
        });
        ui.add_space(30.0);
        ...
        // Set up close button
        self.set_close_button_text("Back".tr());
        self.set_close_button_action(|| {
            UncivGame::current().pop_screen();
        });

        // Set up right side button
        self.set_right_side_button_text("Edit friend".tr());
        self.set_right_side_button_enabled(true);
        self.set_right_side_button_action(|| {
            let friend_name = self.friend_name_text_field.text();
            let player_id = self.player_id_text_field.text();
            ...
            match Uuid::parse_str(&IdChecker::check_and_return_player_uuid(&player_id)) {
                Ok(_) => {
                    ...
                    match friend_list.edit(&self.friend.name, friend_name, player_id) {
                        ErrorType::Name => {
                        ...
                        _ => {
                            ...
                            if let Some(view_friends_list_screen) =
                                new_screen.downcast_ref::<ViewFriendsListScreen>()
                            {
