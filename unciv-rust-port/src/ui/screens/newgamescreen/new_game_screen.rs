use super::{GameOptionsTable, MapOptionsTable, NewGameModCheckHelpers, PlayerPickerTable};
...
impl NewGameScreen {
    pub fn new(game_setup_info: GameSetupInfo) -> Self {
        ...
        let mut screen = Self {
            ...
            map_options_table: Rc::new(RefCell::new(MapOptionsTable::new(game_setup_info.clone()))),
            is_portrait,
        };
    ...
    fn try_update_ruleset(&mut self, update_ui: bool) -> bool {
        ...
        *self.ruleset.borrow_mut() = new_ruleset;

        if update_ui {
            self.new_game_options_table
                .borrow_mut()
                .update_ruleset(&self.ruleset.borrow());
        }
    ...
    pub fn start_game(&mut self) {
        ...
        Concurrency::run(|| {
            if let Some(error_message) = self.get_error_message() {
                Concurrency::run_on_gl_thread(|| {
                    let mut popup = Popup::new(self.clone());
            ...
            let mod_check_result = self
                .new_game_options_table
                .borrow()
                .mod_checkboxes
                .saved_modcheck_result
                .clone();
            self.new_game_options_table
                .borrow_mut()
                .mod_checkboxes
            ...
            if let Some(result) = mod_check_result {
                Concurrency::run_on_gl_thread(|| {
                    AcceptModErrorsPopup::new(
                        self.clone(),
                        result,
                        || self.new_game_options_table.borrow_mut().reset_ruleset(),
                        || {
                            self.game_setup_info
                                .borrow_mut()
                                .game_parameters
                                .accepted_mod_check_errors = Some(result);
                            self.start_game();
                        },
                    )
                    .show();
    ...
    fn get_error_message(&self) -> Option<String> {
        let params = self.game_setup_info.borrow().game_parameters;

        if params.is_online_multiplayer {
            if !self.check_connection_to_multiplayer_server() {
            ...
            {
                if Uuid::parse_str(&IdChecker::check_and_return_player_uuid(&player.player_id))
                    .is_err()
                {
            ...
            if !params.anyone_can_spectate {
                if !params
                    .players
                    .iter()
                    .any(|p| p.player_id == UncivGame::current().settings.multiplayer.user_id)
                {
        ...
        if self.map_options_table.borrow().map_type_select_box.selected
            == MapGeneratedMainType::Custom
        {
            match MapSaver::load_map(&self.game_setup_info.borrow().map_file.unwrap()) {
                Ok(map) => {
                    let incompatibilities = map.get_ruleset_incompatibility(&self.ruleset.borrow());
            ...
            let map_size = self.game_setup_info.borrow().map_parameters.map_size;
            if let Some(message) = map_size
                .fix_undesired_sizes(self.game_setup_info.borrow().map_parameters.world_wrap)
            {
                return Some(message);
            }
        }
    ...
    fn start_new_game(&mut self) {
        let popup = Popup::new(self.clone());
        Concurrency::run_on_gl_thread(|| {
            popup.add_label(Constants::WORKING);
            popup.show();
        });

        let new_game = match self.map_options_table.borrow().get_selected_scenario() {
            None => GameStarter::start_new_game(&self.game_setup_info.borrow()),
            Some(scenario) => {
                let mut game_info = self.game.files.load_game_from_file(&scenario.file);
                // Convert spectator to AI
                if let Some(spectator) = game_info
                    .civilizations
                    .iter_mut()
                ...
                // Update player types
                for player_info in self.game_setup_info.borrow().game_parameters.players.iter() {
                    if let Some(civ) = game_info
                        .civilizations
                        .iter_mut()
            ...
            .is_online_multiplayer
        {
            ...
            match self.game.online_multiplayer.create_game(new_game.clone()) {
                Ok(_) => {
                    self.game.files.autosaves.request_auto_save(new_game);
                }
                Err(FileStorageRateLimitReached {
                    limit_remaining_seconds,
                }) => {
                    Concurrency::run_on_gl_thread(|| {
                        let mut popup = Popup::new(self.clone());
                        popup.add_label(&format!(
        ...
        let world_screen = self.game.load_game(new_game.clone());
