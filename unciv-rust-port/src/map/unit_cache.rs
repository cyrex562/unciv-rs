...
impl MapUnitCache {
    ...
    /// Resets all cache values to their defaults
    pub fn reset(&mut self) {
        *self = Self::new();
    }
    
    /// Adds a unique ability to the cache
    pub fn add_unique(&mut self, unique: String) {
        self.uniques_cache.insert(unique);
    }
    
    /// Checks if the unit has a specific unique ability cached
    pub fn has_unique(&self, unique: &str) -> bool {
        self.uniques_cache.contains(unique)
    }
}
