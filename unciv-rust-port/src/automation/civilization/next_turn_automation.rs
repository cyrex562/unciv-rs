...
impl NextTurnAutomation {
    ...
    pub fn automate_civ_moves(civ_info: &mut Civilization, trade_and_change_state: bool) {
        ...
        if trade_and_change_state && civ_info.is_major_civ() {
            if !civ_info
                ...
                .ruleset
                .mod_options
                .has_unique(UniqueType::DiplomaticRelationshipsCannotChange)
            {
            ...
            civ_info
                ...
                .update_diplomatic_relationship_for_city_state();
        }
    ...
    fn protect_city_states(civ_info: &mut Civilization) {
            ...
            .filter(|civ| !civ.is_defeated() && civ.is_city_state)
        {
            if state
                .city_state_functions
                .other_civ_can_pledge_protection(civ_info)
            {
    ...
    fn bully_city_states(civ_info: &mut Civilization) {
            ...
            .filter(|civ| !civ.is_defeated() && civ.is_city_state)
        {
                ...
                && state.city_state_functions.get_tribute_willingness(civ_info) >= 0
            {
                if state
                    .city_state_functions
                    .get_tribute_willingness(civ_info, true)
                    > 0
                {
    ...
    fn automate_units(civ_info: &mut Civilization) {
        ...
        let sorted_units = civ_info
            .units
            .get_civ_units()
            .sorted_by(|unit| Self::get_unit_priority(unit, is_at_war));

        let cities_requiring_manual_placement = civ_info
            .get_known_civs()
            .iter()
            ...
            .filter(|city| {
                city.get_center_tile()
                    .get_tiles_in_distance(4)
                    .filter(|tile| {
                        tile.military_unit
                            .as_ref()
                            .map_or(false, |unit| unit.civ == civ_info)
                    })
                    .count()
                    > 4
            })
    ...
    pub fn automate_city_conquer(civ_info: &mut Civilization, city: &City) {
        fn our_units_in_range(range: i32, city: &City, civ_info: &Civilization) -> Vec<&MapUnit> {
            city.get_center_tile()
                .get_tiles_in_distance(range)
                .filter_map(|tile| Some(tile.military_unit.as_ref()))
                .filter(|unit| unit.civ == civ_info)
                .collect()
        }

        fn attack_if_possible(unit: &mut MapUnit, tile: &Tile) {
            let attackable_tile = TargetHelper::get_attackable_enemies(
                unit,
        ...
        for unit in our_units_in_range(7, city, civ_info)
            .iter()
        ...
        let siege_units = our_units_in_range(4, city, civ_info)
            .iter()
        ...
        let melee_units = our_units_in_range(5, city, civ_info)
            .iter()
    ...
    pub fn get_closest_cities(civ1: &Civilization, civ2: &Civilization) -> Option<CityDistance> {
        ...
        for civ1_city in &civ1.cities {
            for civ2_city in &civ2.cities {
                let current_distance = civ1_city
                    .get_center_tile()
                    .aerial_distance_to(civ2_city.get_center_tile());
                if min_distance.is_none()
                    || current_distance < min_distance.unwrap().aerial_distance
                {
