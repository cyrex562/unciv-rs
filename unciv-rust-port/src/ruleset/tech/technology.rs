...
impl Technology {
    ...
    pub fn unique_is_requirement_for_this_tech(&self, unique: &Unique) -> bool {
        ...
        unique.modifiers[0].params.get(0).map_or(false, |param| param == &self.name)
    }
    ...
    pub fn has_unique(&self, unique_type: UniqueType) -> bool {
        self.uniques.iter().any(|unique| unique.unique_type == unique_type)
    }
    ...
    pub fn has_unique_with_state(&self, filter: &str, state: &StateForConditionals) -> bool {
        self.uniques.iter().any(|unique| {
            unique.unique_type.to_string() == filter &&
            unique.matches_state(state)
        })
    }

    /// Checks if this technology has a tag unique
    pub fn has_tag_unique(&self, tag: &str) -> bool {
        self.uniques.iter().any(|unique| unique.unique_type.to_string() == tag)
    }
}
...
impl RulesetObject for Technology {
    ...
    fn get_civilopedia_text_lines(&self, ruleset: &Ruleset) -> Vec<crate::ui::screens::civilopediascreen::FormattedLine> {
