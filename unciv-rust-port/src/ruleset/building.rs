...
use crate::models::ruleset::{
    Ruleset, RulesetObject, RulesetStatsObject, INonPerpetualConstruction,
    unique::{Unique, UniqueType, UniqueTarget, LocalUniqueCache, StateForConditionals},
    tile::{TileImprovement, ResourceType},
    unit::BaseUnit,
};
...
impl Building {
    ...
    pub fn get_short_description(&self, multiline: bool, unique_inclusion_filter: Option<&dyn Fn(&Unique) -> bool>) -> String {
    ...
    pub fn is_unavailable_by_settings(&self, game_info: &GameInfo) -> bool {
        ...
        if !game_info.game_parameters.nuclear_weapons_enabled && self.has_unique(UniqueType::EnablesNuclearWeapons) {
            return true;
        }
    ...
    fn is_hidden_by_starting_era(&self, game_info: &GameInfo) -> bool {
        ...
        let starting_era = game_info.ruleset.eras.get(&game_info.game_parameters.starting_era)?;
    ...
    pub fn get_stats(&self, city: &City, local_unique_cache: &LocalUniqueCache) -> Stats {
        ...
        for unique in local_unique_cache.for_city_get_matching_uniques(city, UniqueType::StatsFromObject) {
        ...
        if !self.is_wonder {
            for unique in local_unique_cache.for_city_get_matching_uniques(city, UniqueType::StatsFromBuildings) {
                if self.matches_filter(&unique.params[1], conditional_state) {
                    stats.add(&unique.stats);
                }
            }
    ...
    pub fn get_stat_percentage_bonuses(&self, city: Option<&City>, local_unique_cache: &LocalUniqueCache) -> Stats {
        ...
        for unique in local_unique_cache.for_city_get_matching_uniques(city, UniqueType::StatPercentFromObject) {
            if self.matches_filter(&unique.params[2], conditional_state) {
                stats.add(Stat::value_of(&unique.params[1]), unique.params[0].parse::<f32>().unwrap_or(0.0));
            }
        }

        for unique in local_unique_cache.for_city_get_matching_uniques(city, UniqueType::AllStatsPercentFromObject) {
            if !self.matches_filter(&unique.params[1], conditional_state) {
                continue;
            }
    ...
    pub fn get_production_cost(&self, civ_info: &Civilization, city: Option<&City>) -> i32 {
        ...
        for unique in self.get_matching_uniques(UniqueType::CostIncreasesWhenBuilt, state_for_conditionals) {
            production_cost += civ_info.civ_constructions.built_items_with_increasing_cost.get(&self.name).unwrap_or(&0) * unique.params[0].parse::<i32>().unwrap_or(0) as f32;
        }

        for unique in self.get_matching_uniques(UniqueType::CostIncreasesPerCity, state_for_conditionals) {
            production_cost += civ_info.cities.len() as f32 * unique.params[0].parse::<i32>().unwrap_or(0) as f32;
        }

        for unique in self.get_matching_uniques(UniqueType::CostPercentageChange, state_for_conditionals) {
        ...
        if civ_info.is_city_state() {
            ...
            production_cost *= if self.is_wonder {
                ...
                civ_info.game_info.get_difficulty().ai_building_cost_modifier
            };
        }
    ...
    pub fn can_be_purchased_with_stat(&self, city: Option<&City>, stat: Stat) -> bool {
        ...
        if purchase_reason != PurchaseReason::UniqueAllowed && stat == Stat::Gold && self.is_any_wonder() {
            return false;
        }
        ...
        city.get_matching_uniques(UniqueType::BuyBuildingsByProductionCost, conditional_state)
            .iter()
            .any(|unique| unique.params[1] == stat.name() && self.matches_filter(&unique.params[0], conditional_state)) ||
        city.get_matching_uniques(UniqueType::BuyBuildingsWithStat, conditional_state)
            .iter()
            .any(|unique| {
        ...
        city.get_matching_uniques(UniqueType::BuyBuildingsForAmountStat, conditional_state)
            .iter()
            .any(|unique| {
                unique.params[2] == stat.name() &&
                self.matches_filter(&unique.params[0], conditional_state) &&
    ...
    pub fn get_base_buy_cost(&self, city: &City, stat: Stat) -> Option<f32> {
        ...
        for unique in city.get_matching_uniques(UniqueType::BuyBuildingsIncreasingCost, conditional_state)
            .iter()
            .filter(|unique| {
                unique.params[2] == stat.name() &&
                self.matches_filter(&unique.params[0], conditional_state) &&
            ...
            let count = city.civ.civ_constructions.bought_items_with_increasing_price.get(&self.name).unwrap_or(&0);

            let cost = self.get_cost_for_constructions_increasing_in_price(base_cost, increment, *count) *
                       city.civ.game_info.speed.stat_cost_modifiers.get(&stat).unwrap_or(&1.0);

            costs.push(cost);
        }

        // Add costs from BuyBuildingsByProductionCost uniques
        for unique in city.get_matching_uniques(UniqueType::BuyBuildingsByProductionCost, conditional_state)
            .iter()
            .filter(|unique| unique.params[1] == stat.name() && self.matches_filter(&unique.params[0], conditional_state)) {
            let cost = (self.get_production_cost(&city.civ, Some(city)) * unique.params[2].parse::<i32>().unwrap_or(0)) as f32;
            costs.push(cost);
        }

        // Add cost if BuyBuildingsWithStat unique exists
        if city.get_matching_uniques(UniqueType::BuyBuildingsWithStat, conditional_state)
            .iter()
            .any(|unique| {
                unique.params[1] == stat.name() &&
                self.matches_filter(&unique.params[0], conditional_state) &&
        ...
        for unique in city.get_matching_uniques(UniqueType::BuyBuildingsForAmountStat, conditional_state)
            .iter()
            .filter(|unique| {
                unique.params[2] == stat.name() &&
                self.matches_filter(&unique.params[0], conditional_state) &&
    ...
    pub fn get_stat_buy_cost(&self, city: &City, stat: Stat) -> Option<i32> {
        ...
        for unique in city.get_matching_uniques(UniqueType::BuyBuildingsDiscount) {
            if stat.name() == unique.params[0] && self.matches_filter(&unique.params[1], conditional_state) {
    ...
    pub fn should_be_displayed(&self, city_constructions: &CityConstructions) -> bool {
        ...
        for unique in self.get_matching_uniques(UniqueType::MaxNumberBuildable) {
            if city_constructions.city.civ.civ_constructions.count_constructed_objects(self) >= unique.params[0].parse::<i32>().unwrap_or(0) {
                return false;
            }
        }

        let rejection_reasons: Vec<RejectionReason> = self.get_rejection_reasons(city_constructions).collect();

        if self.has_unique(UniqueType::ShowsWhenUnbuilable, city_constructions.city.state()) &&
           !rejection_reasons.iter().any(|reason| reason.is_never_visible()) {
            return true;
        }

        if rejection_reasons.iter().any(|reason| reason.type_() == RejectionReasonType::RequiresBuildingInSomeCities) &&
           city_constructions.city.civ.game_info.game_parameters.one_city_challenge {
    ...
    pub fn get_rejection_reasons(&self, city_constructions: &CityConstructions) -> impl Iterator<Item = RejectionReason> {
            ...
            match unique.type_() {
                ...
                UniqueType::RequiresPopulation => {
                    ...
                    if required_population > city.population.population {
                        reasons.push(RejectionReasonType::PopulationRequirement.to_instance(&unique.text));
                    }
                }
                ...
                UniqueType::MustNotBeOn => {
                    if city_center.matches_terrain_filter(&unique.params[0], civ) {
                        reasons.push(RejectionReasonType::MustNotBeOnTile.to_instance(&unique.text));
                    }
                }

                UniqueType::MustBeNextTo => {
                    if !city_center.is_adjacent_to(&unique.params[0], civ) && !city_center.matches_filter(&unique.params[0], civ) {
                        reasons.push(RejectionReasonType::MustBeNextToTile.to_instance(&unique.text));
                    }
                }

                UniqueType::MustNotBeNextTo => {
                    if city_center.get_tiles_in_distance(1).iter().any(|tile| tile.matches_filter(&unique.params[0], civ)) {
                        reasons.push(RejectionReasonType::MustNotBeNextToTile.to_instance(&unique.text));
                    }
                }

                UniqueType::MustHaveOwnedWithinTiles => {
                    let distance = unique.params[1].parse::<i32>().unwrap_or(0);
                    if city_center.get_tiles_in_distance(distance)
                        .iter()
                        .none(|tile| tile.matches_filter(&unique.params[0], civ) && tile.get_owner() == civ) {
                ...
                UniqueType::SpaceshipPart => {
                    if !civ.has_unique(UniqueType::EnablesConstructionOfSpaceshipParts) {
                        reasons.push(RejectionReasonType::RequiresBuildingInSomeCity.to_instance("Apollo project not built!"));
                    }
                }

                UniqueType::HiddenBeforeAmountPolicies => {
                    let required_policies = unique.params[0].parse::<i32>().unwrap_or(0);
                    if city_constructions.city.civ.get_completed_policy_branches_count() < required_policies {
                        reasons.push(RejectionReasonType::MorePolicyBranches.to_instance(&unique.text));
                    }
                }
        ...
        if let Some(unique_to) = &self.unique_to {
            if !civ.matches_filter(unique_to, state_for_conditionals) {
                reasons.push(RejectionReasonType::UniqueToOtherNation.to_instance(&format!("Unique to {}", unique_to)));
            }
        }

        if civ.cache.unique_buildings.iter().any(|building| building.replaces.as_ref().map_or(false, |r| r == &self.name)) {
        ...
        for required_tech in self.required_techs() {
            if !civ.tech.is_researched(required_tech) {
                reasons.push(RejectionReasonType::RequiresTech.to_instance(&format!("{} not researched!", required_tech)));
            }
        }

        // All Wonders
        if self.is_any_wonder() {
            if civ.cities.iter().any(|c| c != city_constructions.city && c.city_constructions.is_being_constructed_or_enqueued(&self.name)) {
        ...
        // World Wonders
        if self.is_wonder {
            if civ.game_info.get_cities().iter().any(|c| c.city_constructions.is_built(&self.name)) {
        ...
        if self.is_national_wonder {
            if civ.cities.iter().any(|c| c.city_constructions.is_built(&self.name)) {
        ...
        if let Some(required_building) = &self.required_building {
            if !city_constructions.contains_building_or_equivalent(required_building) {
                ...
                reasons.push(RejectionReasonType::RequiresBuildingInThisCity.to_instance(&format!("Requires a [{}] in this city", equivalent_building)));
            }
        }

        for (resource_name, required_amount) in self.get_resource_requirements_per_turn(state_for_conditionals) {
            let available_amount = city_constructions.city.get_available_resource_amount(&resource_name);
            if available_amount < required_amount {
                reasons.push(RejectionReasonType::ConsumesResources.to_instance(&get_need_more_amount_string(&resource_name, required_amount - available_amount)));
            }
        }

        // If we've already paid the unit costs, we don't need to pay it again
        if city_constructions.get_work_done(&self.name) == 0 {
            for (resource_name, amount) in self.get_stockpiled_resource_requirements(state_for_conditionals) {
                let available_resources = city_constructions.city.get_available_resource_amount(&resource_name);
                if available_resources < amount {
                    reasons.push(RejectionReasonType::ConsumesResources.to_instance(&get_need_more_amount_string(&resource_name, amount - available_resources)));
                }
            }
        }

        if let Some(required_nearby_improved_resources) = &self.required_nearby_improved_resources {
            let contains_resource_with_improvement = city_constructions.city.get_workable_tiles()
                .iter()
                .any(|tile| {
                    tile.resource.is_some() &&
                    required_nearby_improved_resources.contains(&tile.resource.unwrap()) &&
                    tile.get_owner() == civ &&
                    ((tile.get_unpillaged_improvement().is_some() && tile.tile_resource.is_improved_by(tile.improvement.as_ref().unwrap())) ||
                     tile.is_city_center() ||
                     (tile.get_unpillaged_tile_improvement().map_or(false, |imp| imp.is_great_improvement()) &&
                      tile.tile_resource.resource_type == ResourceType::Strategic)
                    )
                });

            if !contains_resource_with_improvement {
                reasons.push(RejectionReasonType::RequiresNearbyResource.to_instance(&format!("Nearby {:?} required", required_nearby_improved_resources)));
            }
        }
    ...
    fn not_met_rejections(&self, unique: &Unique, city_constructions: &CityConstructions, built: bool) -> Vec<RejectionReason> {
        ...
        for conditional in &unique.modifiers() {
            ...
            if Conditionals::conditional_applies(unique, conditional, city_constructions.city.state()) {
                continue;
            }
            ...
            match conditional.type_() {
                UniqueType::ConditionalBuildingBuiltAmount => {
                    ...
                    let number_of_cities = civ.cities.iter()
                        .filter(|city| {
                    ...
                    if number_of_cities < amount as usize {
                        reasons.push(RejectionReasonType::RequiresBuildingInSomeCities.to_instance(
                            &format!("Requires a [{}] in at least [{}] of [{}] cities ({}/{})",
                                    building, amount, city_filter, number_of_cities, number_of_cities)
                        ));
                    }
                }
                ...
                UniqueType::ConditionalBuildingBuiltAll => {
                    ...
                    if civ.cities.iter().any(|city| {
                        ...
                        reasons.push(RejectionReasonType::RequiresBuildingInAllCities.to_instance(
                            &format!("Requires a [{}] in all [{}] cities", building, city_filter)
                        ));
                    }
                }
                ...
                _ => {
                    if built {
                        reasons.push(RejectionReasonType::CanOnlyBeBuiltInSpecificCities.to_instance(&unique.text));
                    } else {
                        reasons.push(RejectionReasonType::ShouldNotBeDisplayed.to_instance());
                    }
                }
    ...
    pub fn is_buildable(&self, city_constructions: &CityConstructions) -> bool {
        self.get_rejection_reasons(city_constructions).next().is_none()
    }
    ...
    pub fn construct(&self, city_constructions: &mut CityConstructions) {
        ...
        if civ_info.game_info.space_resources.contains(&self.name) {
            civ_info.victory_manager.currents_spaceship_parts.add(&self.name, 1);
        }
    ...
    pub fn matches_filter(&self, filter: &str, state: Option<&StateForConditionals>) -> bool {
        MultiFilter::multi_filter(filter, |it| {
            *self.cached_matches_filter_result.entry(it.to_string()).or_insert_with(|| {
    ...
    pub fn is_stat_related(&self, stat: Stat, city: Option<&City>) -> bool {
        if let Some(city) = city {
            if self.get_stats(city, &LocalUniqueCache::new(false)).get(stat) > 0.0 {
                return true;
            }

            if self.get_stat_percentage_bonuses(Some(city), &LocalUniqueCache::new(false)).get(stat) > 0.0 {
                return true;
            }
        } else {
            ...
            if self.get_matching_uniques(UniqueType::Stats, None).iter().any(|unique| unique.stats.get(stat) > 0.0) {
                return true;
            }

            if self.get_stat_percentage_bonuses(None, &LocalUniqueCache::new(false)).get(stat) > 0.0 {
                return true;
            }
        }

        if self.get_matching_uniques(UniqueType::StatsFromTiles, None).iter().any(|unique| unique.stats.get(stat) > 0.0) {
            return true;
        }

        if self.get_matching_uniques(UniqueType::StatsPerPopulation, None).iter().any(|unique| unique.stats.get(stat) > 0.0) {
            return true;
        }
    ...
    pub fn has_create_one_improvement_unique(&self) -> bool {
        if self.has_creates_one_improvement_unique.is_none() {
            self.has_creates_one_improvement_unique = Some(self.has_unique(UniqueType::CreatesOneImprovement));
        }
    ...
    fn get_improvement_to_create(&self, ruleset: &Ruleset) -> Option<&TileImprovement> {
        ...
        if self.improvement_to_create.is_none() {
            let improvement_unique = self.get_matching_uniques(UniqueType::CreatesOneImprovement, None).first()?;
            self.improvement_to_create = Some(ruleset.tile_improvements.get(&improvement_unique.params[0])?.clone());
        }
    ...
    pub fn get_improvement_to_create_for_civ(&self, ruleset: &Ruleset, civ_info: &Civilization) -> Option<&TileImprovement> {
    ...
    pub fn get_resource_requirements_per_turn(&self, state: Option<&StateForConditionals>) -> HashMap<String, i32> {
        let uniques = self.get_matching_uniques(UniqueType::ConsumesResources, state.unwrap_or(&StateForConditionals::empty_state()));
    ...
    pub fn get_stockpiled_resource_requirements(&self, state: Option<&StateForConditionals>) -> HashMap<String, i32> {
        let uniques = self.get_matching_uniques(UniqueType::ConsumesResourcesOnConstruction, state.unwrap_or(&StateForConditionals::empty_state()));
    ...
    fn get_cost_for_constructions_increasing_in_price(&self, base_cost: i32, increment: i32, count: i32) -> f32 {
...
impl INonPerpetualConstruction for Building {
    ...
    fn get_resource_requirements_per_turn(&self, state: Option<&StateForConditionals>) -> HashMap<String, i32> {
        self.get_resource_requirements_per_turn(state)
    }

    fn get_stockpiled_resource_requirements(&self, state: Option<&StateForConditionals>) -> HashMap<String, i32> {
