        ...
        for civ in civ_iteration_order {
            if let Some(points) = points_by_civ.get(&civ) {
                ...
                for i in 1..simplified_scaled_points.len() {
                    ...
                    self.draw_line(ui, a.x, a.y, b.x, b.y, civ_line_color, self.chart_line_width);
                    ...
                    if i == simplified_scaled_points.len() - 1 && self.selected_civ == civ && last_turn_data_points.contains_key(&civ) {
                        let (icon, _) = VictoryScreenCivGroup::get_civ_image_and_colors(&civ, &self.viewing_civ, VictoryScreenCivGroup::DefeatedPlayerStyle::Regular);
                        ui.put(
                            egui::pos2(b.x, b.y),
