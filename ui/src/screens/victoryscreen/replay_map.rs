...
impl ReplayMap {
    ...
    fn draw_grid(&self, ui: &mut Ui) {
        ...
        for x in 0..self.tile_map.width {
            let start = egui::pos2(
                ...
                self.bounds.top() + self.offset.y
            );
            let end = egui::pos2(
                self.bounds.left() + x as f32 * self.tile_size * self.scale + self.offset.x,
                self.bounds.bottom() + self.offset.y
            );
        ...
        for y in 0..self.tile_map.height {
            let start = egui::pos2(
                self.bounds.left() + self.offset.x,
                self.bounds.top() + y as f32 * self.tile_size * self.scale + self.offset.y
            );
            let end = egui::pos2(
                self.bounds.right() + self.offset.x,
                self.bounds.top() + y as f32 * self.tile_size * self.scale + self.offset.y
            );
    ...
    fn draw_tiles(&self, ui: &mut Ui) {
        for tile in self.tile_map.tiles.values() {
            let pos = egui::pos2(
                self.bounds.left() + tile.position.x as f32 * self.tile_size * self.scale + self.offset.x,
                self.bounds.top() + tile.position.y as f32 * self.tile_size * self.scale + self.offset.y
            );
    ...
    fn draw_civilizations(&self, ui: &mut Ui) {
        for civ in self.tile_map.civilizations.values() {
            if self.use_actual_color(civ) {
                let pos = egui::pos2(
                    self.bounds.left() + civ.capital_location.x as f32 * self.tile_size * self.scale + self.offset.x,
                    self.bounds.top() + civ.capital_location.y as f32 * self.tile_size * self.scale + self.offset.y
                );
    ...
    fn draw_ui(&self, ui: &mut Ui) {
        // Draw the turn label
        ui.put(
            egui::pos2(self.bounds.left() + 10.0, self.bounds.top() + 10.0),
            egui::Label::new(RichText::new(tr("Turn {}", self.turn)).color(Color32::WHITE))
        );

        // Draw the civilization label
        ui.put(
            egui::pos2(self.bounds.left() + 10.0, self.bounds.top() + 30.0),
            egui::Label::new(RichText::new(tr("Civilization: {}", self.selected_civ.nation.name)).color(Color32::WHITE))
        );
    }
