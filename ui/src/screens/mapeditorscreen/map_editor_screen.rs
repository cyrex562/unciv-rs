...
impl MapEditorScreen {
    pub fn new(map: Option<TileMap>) -> Self {
        let (tile_map, ruleset) = if let Some(map) = map {
            let ruleset = map
                .ruleset
                .clone()
                .unwrap_or_else(|| RulesetCache::get_complex_ruleset(&map.map_parameters));
            ...
            (
                Rc::new(RefCell::new(tile_map)),
                Rc::new(RefCell::new(ruleset)),
            )
        };

        let new_map_parameters = Self::get_default_parameters();
        let description_text_field =
            UncivTextField::new("Enter a description for the users of this map");
        ...
        let mut screen = Self {
            ...
            tabs: Rc::new(RefCell::new(MapEditorMainTabs::new(Rc::new(RefCell::new(
                self.clone(),
            ))))),
        ...
        screen.tabs = Rc::new(RefCell::new(MapEditorMainTabs::new(Rc::new(RefCell::new(
            screen.clone(),
        )))));
        MapEditorToolsDrawer::new(
            screen.tabs.clone(),
            screen.stage.clone(),
            screen.map_holder.clone(),
        );
        ...
        screen.global_shortcuts.add(
            KeyCharAndCode::ctrl('n'),
            Box::new(move || {
                select_generate_page(0);
            }),
        );
        screen.global_shortcuts.add(
            KeyCharAndCode::ctrl('g'),
            Box::new(move || {
    ...
    fn get_default_parameters() -> MapParameters {
        let last_setup = UncivGame::current()
            .settings
            .last_game_setup
        ...
        parameters
            .mods
            .retain(|mod_name| !RulesetCache::get_sorted_base_rulesets().contains(mod_name));
    ...
    pub fn save_default_parameters(parameters: &MapParameters) {
        ...
        let game_parameters = settings
            .last_game_setup
            .as_ref()
            .map(|setup| setup.game_parameters.clone())
            .unwrap_or_else(GameParameters::new);

        settings.last_game_setup = Some(GameSetupInfo::new(game_parameters, parameters.clone()));
    ...
    fn set_world_wrap_fix_odd_width(&mut self, new_value: bool) {
        ...
        if map_parameters.world_wrap
            && map_parameters.map_size.width % 2 != 0
            && map_parameters.shape == MapShape::Rectangular
        {
    ...
    fn new_map_holder(&self) -> Rc<RefCell<EditorMapHolder>> {
        ...
        UncivGame::current().translations.translation_active_mods =
            self.ruleset.borrow().mods.clone();
        ...
        self.stage
            .add_listener(KeyboardPanningListener::new(new_holder.clone(), false));

        if Gdx::app().type_() == Application::ApplicationType::Desktop {
            new_holder.borrow_mut().is_auto_scroll_enabled =
                UncivGame::current().settings.map_auto_scroll;
        }
    ...
    pub fn load_map(&mut self, map: TileMap, new_ruleset: Option<Ruleset>, select_page: i32) {
        ...
        self.ruleset =
            Rc::new(RefCell::new(new_ruleset.unwrap_or_else(|| {
    ...
    fn overlay_alpha_changed(&mut self, value: f32) {
        if let Some(image) = &mut self.image_overlay {
            image.color.a = value;
        }
    }

    fn add_overlay_to_map_holder(&mut self, new_holder_content: Group) {
        self.clear_overlay_images();
        if self.overlay_file.is_none() {
            return;
        }

        match Texture::new(self.overlay_file.as_ref().unwrap()) {
            Ok(texture) => {
                ...
                image.color.a = self.overlay_alpha;
                new_holder_content.add_actor(image.clone());

                self.image_overlay = Some(image);
            }
            Err(ex) => {
                Log::error("Invalid overlay image", ex);
                self.overlay_file = None;
                ToastPopup::new("Invalid overlay image", self.stage.clone(), 3000).open();
                self.tabs.borrow_mut().options.update();
            }
        }
    }
