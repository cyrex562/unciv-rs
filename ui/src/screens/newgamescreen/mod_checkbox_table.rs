...
use egui::{Checkbox, Color32, ScrollArea, Ui};
...
impl ModCheckboxTable {
    ...
    pub fn set_base_ruleset(&mut self, new_base_ruleset_name: String) {
        ...
        let compatible_mods: Vec<&ModWithCheckbox> = self
            .mod_widgets
            .iter()
    ...
    fn complex_mod_check_returns_errors(&mut self) -> bool {
        ...
        let complex_mod_link_check =
            RulesetCache::check_combined_mod_links(&self.mods.borrow(), &self.base_ruleset_name);
    ...
    fn deselect_incompatible_mods(&mut self, skip_checkbox: Option<&Checkbox>) {
        ...
        for mod_widget in &mut self.mod_widgets {
            ...
            if !ModCompatibility::meets_all_requirements(
                &mod_widget.mod_,
                &self.base_ruleset,
                self.get_selected_mods(),
            ) {
                mod_widget.widget.checked = false;
                self.mods.borrow_mut().remove(&mod_widget.mod_.name);
            }
        }
        self.disable_change_events = false;
    }

    fn disable_incompatible_mods(&mut self) {
        for mod_widget in &mut self.mod_widgets {
            let enable = ModCompatibility::meets_all_requirements(
                &mod_widget.mod_,
                &self.base_ruleset,
                self.get_selected_mods(),
            );
    ...
    pub fn change_game_parameters(&mut self, new_game_parameters: &GameParameters) {
        *self.mods.borrow_mut() = new_game_parameters.mods.clone();
    }
    ...
    pub fn show(&mut self, ui: &mut Ui) {
        ...
        ExpanderTab::new("Extension mods", "NewGameExpansionMods", false).show(ui, |ui| {
            ui.add_space(5.0);
            for mod_widget in compatible_mods {
                if ui
                    .checkbox(&mut mod_widget.widget.checked, mod_widget.mod_.name.clone())
                    .clicked()
                {
                    self.check_box_changed(&mut mod_widget.widget, &mod_widget.mod_);
                }
                ui.add_space(5.0);
            }
        });
