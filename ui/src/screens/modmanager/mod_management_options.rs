...
impl ModManagementOptions {
    ...
    fn init_ui(&mut self, mod_management_screen: &ModManagementScreen) {
        ...
        let mut sort_installed_select = TranslatedSelectBox::new(
            SortType::entries()
                ...
                .collect(),
            self.sort_installed.label.clone(),
        );
        sort_installed_select.on_change(Box::new(move || {
            self.sort_installed = SortType::from_select_box(&sort_installed_select);
        ...
        let mut sort_online_select = TranslatedSelectBox::new(
            SortType::entries()
                ...
                .map(|sort| sort.label.clone())
                .collect(),
            self.sort_online.label.clone(),
        );
        sort_online_select.on_change(Box::new(move || {
            self.sort_online = SortType::from_select_box(&sort_online_select);
        ...
        let mut category_select = TranslatedSelectBox::new(
            ModCategories::as_sequence()
                ...
                .collect(),
            self.category.label.clone(),
        );
        category_select.on_change(Box::new(move || {
            self.category = ModCategories::from_select_box(&category_select);
            mod_management_screen.refresh_installed_mod_table();
            mod_management_screen.refresh_online_mod_table();
        }));

        self.expander = ExpanderTab::new(
            tr("Sort and Filter"),
            DEFAULT_FONT_SIZE,
            false,
            2.5,
            ...
            360.0,
            Some(Box::new(move || {
                if let Some(event) = &self.expander_change_event {
                    event();
                }
            })),
        );
        ...
        self.expander.add(filter_table);

        search_icon.set_touchable(true);
        search_icon.on_activation(Box::new(move || {
            if self.expander.is_open() {
                mod_management_screen.refresh_installed_mod_table();
                mod_management_screen.refresh_online_mod_table();
            } else {
                mod_management_screen
                    .stage
                    .set_keyboard_focus(&self.text_field);
            }
            self.expander.toggle();
        }));
    ...
    pub fn get_filter(&self) -> Filter {
        Filter {
            text: self.text_field.clone(),
            topic: self.category.topic.clone(),
        }
    }

    pub fn get_installed_header(&self) -> String {
        format!("{} {}", tr("Current mods"), self.sort_installed.symbols)
    }

    pub fn get_online_header(&self) -> String {
        format!("{} {}", tr("Downloadable mods"), self.sort_online.symbols)
    }

    pub fn installed_header_clicked(&mut self) {
        loop {
            self.sort_installed = self.sort_installed.next();
            if self.sort_installed != SortType::Stars {
                break;
            }
        }
        self.sort_installed_select
            .set_selected(TranslatedSelectBox::TranslatedString::new(
                self.sort_installed.label.clone(),
            ));
        mod_management_screen.refresh_installed_mod_table();
    }

    pub fn online_header_clicked(&mut self) {
        self.sort_online = self.sort_online.next();
        self.sort_online_select
            .set_selected(TranslatedSelectBox::TranslatedString::new(
                self.sort_online.label.clone(),
            ));
        mod_management_screen.refresh_online_mod_table();
